...
**Fact 2: phi_Q(g) = W_0(diag(g,1) u_Q) is nonzero for W_0 = new vector.**
...
span all of K(Pi)|_{GL_n}. We use this in two directions:

**Direction 1 (L_phi divides L(s, Pi x pi)):** Phi(phi) ⊆ Phi_full
(the full ideal generated by all W and V), so L_phi · R ⊆ L(s, Pi x pi) · R,
giving L(s, Pi x pi) | L_phi (in a PID, (a) ⊆ (b) iff b | a).

**Direction 2 (L(s, Pi x pi) divides L_phi):** For any nonzero phi' in
K(Pi)|_{GL_n}, write phi' = sum_i c_i R(g_i) phi (finite linear combination
of GL_n-translates, possible by irreducibility). For each V:
...
Taking the union over all phi': the full ideal L(s, Pi x pi) · R =
∪_{phi'} Phi(phi') ⊆ L_phi · R. So L_phi | L(s, Pi x pi).

Combining both directions: L_phi = L(s, Pi x pi) (up to units in R). ∎
