...
**Step 7c: Global Hamiltonian isotopy.** Each vertex smoothing and each edge
smoothing can be parameterized by a width parameter t (delta for vertices,
eps for edges). As t → 0, the smoothing region shrinks and the smooth
Lagrangian K_t converges to the original polyhedral surface K = K_0 in
the C^0 topology. The smooth Lagrangian surfaces K_t for t > 0 form a
Hamiltonian isotopy (each local smoothing is generated by a compactly
supported Hamiltonian function — the generating function S_eps for edges,
and the product of curve-smoothing Hamiltonians for vertices; their
composition is a Hamiltonian isotopy by McDuff-Salamon, *Introduction
to Symplectic Topology*, 3rd ed., Proposition 3.17).
