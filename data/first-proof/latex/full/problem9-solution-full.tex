...
Does there exist a polynomial map F: \(R^{\mNumber{81} \ast n^\mNumber{4}}\) -\textgreater{} \(R^{N}\) satisfying:
...
form on \(R^\mNumber{4}\)) on four camera row vectors.

\textbf{Key observation (bilinear form reduction):} Fix two camera-row pairs
(gamma, k) and (delta, l), giving vectors \(c = a^{\gamma}_{k}\) and \(d = a^{\delta}_{l} \in R^\mNumber{4}.\) Then the map:

\[\omega(p, q) = det[p; q; c; d]\]

is an alternating bilinear form on \(R^\mNumber{4}.\) Since c wedge d is a simple 2-form,
...
\(\omega\) induces a non-degenerate alternating form on V/span\{c,d\} = \(R^\mNumber{2}.\)
...
3 vectors \(q_\mNumber{1},\) \(q_\mNumber{2},\) \(q_\mNumber{3} \in R^\mNumber{4}:\)
...
transformations preserve rank: rank(M) = rank(Omega) = \(\mNumber{2} < \mNumber{3}\), so det(M) = 0.
