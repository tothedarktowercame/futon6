...
Let \(n \ge \mNumber{5}\). Let \(A^{(\mNumber{1})}, ..., A^{(n)} \in R^{\mNumber{3} \times \mNumber{4}}\) be Zariski-generic matrices.
For \(\alpha, \beta, \gamma, \delta \in [n],\) construct \(Q^{(\alpha \beta \gamma \delta)} \in R^{\mNumber{3} \times \mNumber{3}x\mNumber{3} \times \mNumber{3}}\) with entry:
...
Does there exist a polynomial map F: \(R^{\mNumber{81} \ast n^{\mNumber{4}}} \to \mathbb{R}^{N}\) satisfying:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  F does not depend on \(A^{(\mNumber{1})}, ..., A^{(n)}\)
\item
  The degrees of the coordinate functions of F do not depend on n
\item
  For \(\lambda \in R^{n \times n \times n \times n}\) with \(\lambda_{abgd} \neq \mNumber{0}\) precisely when
...
form on \(\mathbb{R}^{\mNumber{4}}\)) on four camera row vectors.

\textbf{Key observation (bilinear form reduction):} Fix two camera-row pairs
\((\gamma, k)\) and \((\delta, l)\), giving vectors \(c = a^{(\gamma)}_{k}\) and \(d = a^{(\delta)}_{l} \in \mathbb{R}^{\mNumber{4}}.\) Then the map:

\[\omega(p, q) = \det[p; q; c; d]\]

is an alternating bilinear form on \(\mathbb{R}^{\mNumber{4}}\). Since c wedge d is a simple 2-form,
...
\(\omega\) induces a non-degenerate alternating form on \(V/span{c,d} = \mathbb{R}^{\mNumber{2}}\).
...
Since \(\omega\) has rank 2, for ANY choice of 3 vectors \(p_{\mNumber{1}}, p_{\mNumber{2}}, p_{\mNumber{3}}\) and
3 vectors \(q_{\mNumber{1}}, q_{\mNumber{2}}, q_{\mNumber{3}} \in \mathbb{R}^{\mNumber{4}}\):

\[\begin{aligned}
\det | \omega(p_\mNumber{1},q_\mNumber{1}) \omega(p_\mNumber{1},q_\mNumber{2}) \omega(p_\mNumber{1},q_\mNumber{3}) | \\
| \omega(p_\mNumber{2},q_\mNumber{1}) \omega(p_\mNumber{2},q_\mNumber{2}) \omega(p_\mNumber{2},q_\mNumber{3}) | = \mNumber{0} \\
| \omega(p_\mNumber{3},q_\mNumber{1}) \omega(p_\mNumber{3},q_\mNumber{2}) \omega(p_\mNumber{3},q_\mNumber{3}) |
\end{aligned}\]

(A rank-2 bilinear form has all 3x3 minors vanishing.)

In terms of the Q tensors: choosing \(p_{m} = a^{(\alpha_{m})}_{i_{m}}\) and
\(q_{n} = a^{(\beta_{n})}_{j_{n}}\), this becomes:

\[\det [Q^{(\alpha_{m}, \beta_{n}, \gamma, \delta)}_{i_{m}, j_{n}, k, l}]_{\mNumber{3} \times \mNumber{3}} = \mNumber{0}\]

for any choice of \((\alpha_{\mNumber{1}}, i_{\mNumber{1}}), (\alpha_{\mNumber{2}}, i_{\mNumber{2}}), (\alpha_{\mNumber{3}}, i_{\mNumber{3}})\) and
\((\beta_{\mNumber{1}}, j_{\mNumber{1}}), (\beta_{\mNumber{2}}, j_{\mNumber{2}}), (\beta_{\mNumber{3}}, j_{\mNumber{3}})\) and fixed \((\gamma, k), (\delta, l).\)
...
\(\Lambda = (u_{\alpha_{m}})_{m}\,(v_{\beta_{n}})_{n}^{T}\) (scaled by the constant \(w_{\gamma} x_{\delta}\)).
...
transformations preserve rank: \(\mOpName{rank}(M) = \mOpName{rank}(\Omega) = \mNumber{2} < \mNumber{3}\), so \(\det(M) = \mNumber{0}\).
...
\[P(A^{(\mNumber{1})}, ..., A^{(n)}) = \det [T^{(\alpha_{m}, \beta_{n}, \gamma, \delta)}_{i_{m}, j_{n}, k, l}]_{\mNumber{3} \times \mNumber{3}}\]
...
\textbf{Explicit witness (\(n = \mNumber{5}\)).} Choose 5 cameras \(A^{(i)}\) as rows of generic
\(\mNumber{3} \times \mNumber{4}\) matrices with rational entries:

\[\begin{aligned}
A^{(\mNumber{1})} = [[\mNumber{1},\mNumber{0},\mNumber{0},\mNumber{1}],[\mNumber{0},\mNumber{1},\mNumber{0},\mNumber{1}],[\mNumber{0},\mNumber{0},\mNumber{1},\mNumber{1}]] \\
A^{(\mNumber{2})} = [[\mNumber{1},\mNumber{1},\mNumber{0},\mNumber{0}],[\mNumber{0},\mNumber{1},\mNumber{1},\mNumber{0}],[\mNumber{0},\mNumber{0},\mNumber{1},\mNumber{1}]] \\
A^{(\mNumber{3})} = [[\mNumber{1},\mNumber{0},\mNumber{1},\mNumber{0}],[\mNumber{1},\mNumber{1},\mNumber{0},\mNumber{0}],[\mNumber{0},\mNumber{1},\mNumber{0},\mNumber{1}]] \\
A^{(\mNumber{4})} = [[\mNumber{0},\mNumber{1},\mNumber{1},\mNumber{1}],[\mNumber{1},\mNumber{0},\mNumber{1},\mNumber{1}],[\mNumber{1},\mNumber{1},\mNumber{0},\mNumber{1}]] \\
A^{(\mNumber{5})} = [[\mNumber{1},\mNumber{2},\mNumber{3},\mNumber{4}],[\mNumber{4},\mNumber{3},\mNumber{2},\mNumber{1}],[\mNumber{1},\mNumber{1},\mNumber{1},\mNumber{1}]]
\end{aligned}\]

Choose a non-rank-1 scaling: \(\lambda_{abgd} = \mNumber{1}\) for all non-identical (\(a,b,g,d\))
except \(\lambda_{\mNumber{1},\mNumber{2},\mNumber{3},\mNumber{4}} = \mNumber{2}\).

\textbf{Non-rank-1 verification.} If \(\lambda\) were rank-1, then \(\lambda_{abgd} = u_{a} v_{b} w_{g} x_{d}.\)
From \(\lambda_{\mNumber{1},\mNumber{2},\mNumber{3},\mNumber{4}} = \mNumber{2}\) and \(\lambda_{\mNumber{2},\mNumber{2},\mNumber{3},\mNumber{4}} = \mNumber{1}\) we get \(u_{\mNumber{1}}/u_{\mNumber{2}} = \mNumber{2}.\) But from
\(\lambda_{\mNumber{1},\mNumber{1},\mNumber{3},\mNumber{4}} = \mNumber{1}\) and \(\lambda_{\mNumber{2},\mNumber{1},\mNumber{3},\mNumber{4}} = \mNumber{1}\) we get \(u_{\mNumber{1}}/u_{\mNumber{2}} = \mNumber{1}.\) Contradiction.

Fix \(\gamma = \mNumber{3}, \delta = \mNumber{4}, k = \mNumber{1}, l = \mNumber{1}.\) Choose row triples
\((\alpha_{m}, i_{m}) = (\mNumber{1}, \mNumber{1}), (\mNumber{2}, \mNumber{1}), (\mNumber{5}, \mNumber{1})\) and column triples
\((\beta_{n}, j_{n}) = (\mNumber{2}, \mNumber{1}), (\mNumber{5}, \mNumber{1}), (\mNumber{1}, \mNumber{2}).\) The 3x3 \(\Lambda\) matrix has entries
\(\lambda_{\alpha_{m}, \beta_{n}, \mNumber{3}, \mNumber{4}}\); specifically \(\Lambda_{\mNumber{11}} = \lambda_{\mNumber{1},\mNumber{2},\mNumber{3},\mNumber{4}} = \mNumber{2}\)
with all other entries equal to 1. The \(\Omega\) matrix (unscaled Q values) has
det(\(\Omega) = \mNumber{0}\) (confirming rank-2), but the Hadamard product \(M = \Lambda\) ∘ \(\Omega\)
satisfies det(M) \(= -\mNumber{24} \neq \mNumber{0}\) (verified by direct computation of nine 4x4
...
\(\mOpName{rank}(\Lambda ∘ \Omega) \le \mOpName{rank}(\Lambda) \ast \mOpName{rank}(\Omega)\) provides context but is
...
The construction in Section 2-4 tests the rank-1 condition on the \((\mNumber{1}, \mNumber{2})\)-
...
\tightlist
\item
  Fix modes \((\mNumber{3}, \mNumber{4})\), vary modes \((\mNumber{1}, \mNumber{2})\): test \(\lambda_{(\alpha, \beta), (\gamma, \delta)}\)
\item
  Fix modes \((\mNumber{2}, \mNumber{4})\), vary modes \((\mNumber{1}, \mNumber{3})\): test \(\lambda_{(\alpha, \gamma), (\beta, \delta)}\)
\item
  Fix modes \((\mNumber{2}, \mNumber{3})\), vary modes \((\mNumber{1}, \mNumber{4})\): test \(\lambda_{(\alpha, \delta), (\beta, \gamma)}\)
\end{itemize}
...
\tightlist
\item
  Mode-\((\mNumber{1}, \mNumber{2})\) vs \((\mNumber{3}, \mNumber{4})\) rank 1 gives \(\lambda_{abgd} = f_{ab} g_{gd}.\)
\item
  Mode-\((\mNumber{1}, \mNumber{3})\) vs \((\mNumber{2}, \mNumber{4})\) rank 1 gives \(\lambda_{abgd} = h_{ag} k_{bd}.\)
  From the first: \(\lambda_{a\mNumber{1},b\mNumber{1},g,d} / \lambda_{a\mNumber{2},b\mNumber{2},g,d} = f_{a\mNumber{1},b\mNumber{1}} / f_{a\mNumber{2},b\mNumber{2}},\)
  independent of g, d. From the second:
  \(\lambda_{a,b\mNumber{1},g\mNumber{1},d} / \lambda_{a,b\mNumber{2},g\mNumber{1},d} = k_{b\mNumber{1},d} / k_{b\mNumber{2},d},\)
...
\[F_{choice} = \det [T^{(\alpha_{m}, \beta_{n}, \gamma, \delta)}_{i_{m}, j_{n}, k, l}]_{m,n = \mNumber{1},\mNumber{2},\mNumber{3}}\]
...
\tightlist
\item
  Three ``row'' camera-row pairs \((\alpha_{m}, i_{m})\) for \(m = \mNumber{1}\),2,3
\item
  Three ``column'' camera-row pairs \((\beta_{n}, j_{n})\) for \(n = \mNumber{1}\),2,3
\item
...
\item
  \textbf{Characterization:} \(F = \mNumber{0}\) iff all three matricizations of \(\lambda\) have
...
(there are \(O(n^\mNumber{8})\) choices for the 3x3 minors from the \((\mNumber{1}, \mNumber{2})\) vs \((\mNumber{3}, \mNumber{4})\)
...
The rank-1 scaling \(\lambda = u \otimes v \otimes w \otimes x\) corresponds to the natural
...
\item
  PlanetMath: \(\text{tensor} rank'' /\)simple tensor'' (SimpleTensor) --- rank-1 tensors
\item
