...
\(R^{\mNumber{3} \times \mNumber{3}x\mNumber{3} \times \mNumber{3}}\) with entry:
...
Now consider the scaled tensors \(T^{abgd} = \lambda_{abgd} Q^{abgd}.\)The 3x3 matrix becomes:
...
For a rank-1 matrix \(\Lambda\), the Hadamard product \(M = \Lambda \circ \Omega\)equals
\(\operatorname{diag}(u) \ast \Omega \ast \operatorname{diag}(v)\) (up to the scalar \(w_{\gamma} x_{\delta}\)). Since similar
transformations preserve rank: \(\mOpName{rank}(M) = \mOpName{rank}(\Omega) = \mNumber{2} < \mNumber{3}\), so det(M) = 0.
...
\textbf{Non-rank-1 verification.} If \(\lambda\) were rank-1, then \(\lambda_{abgd} = u_{a} v_{b} w_{g} x_{d}.\)From \(\lambda_{\mNumber{1},\mNumber{2},\mNumber{3},\mNumber{4}} = \mNumber{2}\) and \(\lambda_{\mNumber{2},\mNumber{2},\mNumber{3},\mNumber{4}} = \mNumber{1}\) we get \(u_\mNumber{1}/u_\mNumber{2} = \mNumber{2}\). But from
...
det(Omega) = 0 (confirming rank-2), but the Hadamard product \(M = \Lambda \circ \Omega\)satisfies det(M) = -24 != 0 (verified by direct computation of nine 4x4
...
Hadamard product \(\Lambda \circ \Omega,\)where \(\Lambda\) carries the scaling entries
and \(\Omega\) carries the bilinear form values. The upper bound
\$\textbackslash mOpName\{rank\}(Lambda \(\circ\) Omega)\$ \textless= \(\mOpName{rank}(\Lambda) \ast \mOpName{rank}(\Omega)\) provides context but is
...
\item
  Mode-(1,3) vs (2,4) rank 1 gives \(\lambda_{abgd} = h_{ag} k_{bd}.\)From the first: \(\lambda_{a \backslash mNumber{\mNumber{1}},b \backslash mNumber{\mNumber{1}},g,d} / \lambda_{a \backslash mNumber{\mNumber{2}},b \backslash mNumber{\mNumber{2}},g,d} = f_{a \backslash mNumber{\mNumber{1}},b \backslash mNumber{\mNumber{1}}} / f_{a \backslash mNumber{\mNumber{2}},b \backslash mNumber{\mNumber{2}}},\)independent of g, d. From the second:
  \(\lambda_{a,b \backslash mNumber{\mNumber{1}},g \backslash mNumber{\mNumber{1}},d} / \lambda_{a,b \backslash mNumber{\mNumber{2}},g \backslash mNumber{\mNumber{1}},d} = k_{b \backslash mNumber{\mNumber{1}},d} / k_{b \backslash mNumber{\mNumber{2}},d},\)independent of a, g1. Cross-referencing these separations forces
...
(there are \(O(n^\mNumber{8})\) choices for the 3x3 minors from the (1,2) vs (3,4)
...
geometry: four collinear points in \(P^\mNumber{3}\) impose a codimension constraint on
