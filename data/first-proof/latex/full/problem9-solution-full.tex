...
det(Omega) \(=\) 0 (confirming rank-2), but the Hadamard product \(M = \Lambda \circ \Omega\)
satisfies det(M) \(= -\mNumber{24} \neq\) 0 (verified by direct computation of nine 4x4
...
\$\textbackslash mOpName\{rank\}(Lambda \(\circ\) Omega)\$ \(\le \mOpName{rank}(\Lambda) \ast \mOpName{rank}(\Omega)\) provides context but is
