...
\[[(Z \times K)^{T} D (Z \times K) + \lambda (I_{r} \times K)] \mOpName{vec}(W) = (I_{r} \times K) \mOpName{vec}(B),\]

where \(D = S S^{T}\) is the observation projector (q observed entries out of
\(N = nM), B = T Z,\) and n, \(r \ll q \ll N\).
...
x = \mOpName{vec}(W), \\
b_{\tau} = (I_{r} \times K_{\tau}) \mOpName{vec}(B).
\end{aligned}\]
...
\(A_{\tau}\) is an \((nr) \times (nr)\) system. Dense direct factorization costs
...
CG needs only \(y = A_{\tau}\mathbf{x}\), not \(A_{\tau}\) explicitly.

Given \(\mathbf{x} = \mOpName{vec}(V),\; V \in R^{n \times r}:\)
...
\item
  ...
  \[(Z \times K_{\tau}) \mOpName{vec}(V) = \mOpName{vec}(K_{\tau} V \mathbb{Z}^{T}).\]
  ...
  Total \(O(q r)\).
\item
  Form sparse W\textquotesingle{} in \(R^{n \times M}\) from \(u_{l}\). Let \(s = nnz(W') \le q\).
\item
  Adjoint sampled action:

  \[(\mathbb{Z}^{T} \times K_{\tau}) \mOpName{vec}(W') = \mOpName{vec}(K_{\tau} W' Z).\]
...
So \(b_{\tau} = (I_{r} \times K_{\tau}) \mOpName{vec}(B)\) is formed in \(O(q r + n^\mNumber{2} r)\).
...
\[\begin{aligned}
\hat{A} = (I_{r} \times K_{\tau}^{-\mNumber{1}/\mNumber{2}}) A_{\tau} (I_{r} \times K_{\tau}^{-\mNumber{1}/\mNumber{2}}) \\
...
\[\hat{A} ~ c (\mathbb{Z}^{T} Z \times K_{\tau}) + \lambda I.\]

Choose Kron preconditioner in whitened coordinates:

\[\hat{P} = (c Z^{T} Z + \lambda I_{r}) \times I_{n}.\]
...
\[\mathbb{Z}^{T} Z = \text{Hadamard}_{i} (A_{i}^{T} A_{i}),\]
...
\[t = O(\sqrt(\kappa) \log(\mNumber{1}/\epsilon)).\]
...
(1-\(\delta\))P \(\le A_{\tau} \le (\mNumber{1}+\delta\))P holds with \(\delta\) bounded away from 1 when
the sampling pattern satisfies a restricted isometry-type condition:
the restricted isometry holds for the column space of Z \(\otimes K_{\tau}^{\mNumber{1}/\mNumber{2}},\)
i.e. (Z \(\otimes K_{\tau}^{\mNumber{1}/\mNumber{2}})^{T}\) (D - cI) (Z \(\otimes K_{\tau}^{\mNumber{1}/\mNumber{2}})\) is small in
...
concentration yields \(\delta\) bounded away from 1 with high probability. Under this regime, \(\kappa = O(\mNumber{1})\) and PCG converges in
\(O(\log(\mNumber{1}/\epsilon))\) iterations.
...
\tightlist
\item
  \(\text{Cholesky}(K_{\tau})\): \(O(n^\mNumber{3})\)
\item
...
approximation with rank \(p \ll n\), reducing the kernel factorization to \(O(n p^\mNumber{2})\))
