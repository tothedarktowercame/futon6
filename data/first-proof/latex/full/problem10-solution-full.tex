...
\(O((\text{nr})^\mNumber{3}) = O(n^\mNumber{3} r^\mNumber{3})\).

A naive explicit route also materializes \(\Phi = Z \times K_{\tau} \in R^{N \times \text{nr}}\),
...
\item
  ...
  \[(Z \times K_{\tau}) \mOpName{vec}(V) = \mOpName{vec}(K_{\tau} V Z^{T}).\]
  ...
  Total \(O(q r)\).
\item
  Form sparse W\textquotesingle{} in \(R^{n \times M}\) from \(u_{l}\). Let s \(=\) nnz(W\textquotesingle) \(\le\) q.
\item
  Adjoint sampled action:

  \[(Z^{T} \times K_{\tau}) \mOpName{vec}(W') = \mOpName{vec}(K_{\tau} W' Z).\]
...
\[\hat{A} ~ c (Z^{T} Z \times K_{\tau}) + \lambda I.\]
...
\[Z^{T} Z = \text{Hadamard}_{i} (A_{i}^{T} A_{i}),\]
...
(1-delta)P \(\le A_{\tau} \le\) (1+delta)P holds with \(\delta\) bounded away from 1 when
