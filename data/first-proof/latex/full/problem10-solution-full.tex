...
\item
  \(Z^{T}\) Z via Hadamard Grams: \(O(sum_{i} n_{i} r^\mNumber{2})\)
\item
...
dominates (i.e., \(n^\mNumber{3}\) \textgreater{} t(\(n^\mNumber{2}\) \(r \mBridgeOperator{+} q\) r)), the per-ALS-step cost is effectively
...
\[\begin{aligned}
SETUP: \\
K_{\tau} = K + \tau \ast I_{n} \# \tau > \mNumber{0} if K is only PSD \\
L_{K} = cholesky(K_{\tau}) \# O(n^\mNumber{3}) \\
G = hadamard_{product}(A_{i}^{T} A_{i} for i \neq k) \# O(sum_{i} n_{i} r^\mNumber{2}) \\
c = q / N \\
H = c \ast G + \lambda \ast I_{r} \\
L_{H} = cholesky(H) \# O(r^\mNumber{3}) \\
B = sparse_{mttkrp}(T, Z) \# O(qr) \\
b = vec(K_{\tau} @ B) \# O(n^\mNumber{2} r) \\
PCG(A_{\tau} x = b, preconditioner P = H \times K_{\tau}): \\
x\mNumber{0} = \mNumber{0} \\
r\mNumber{0} = b \\
z\mNumber{0} = precond_{solve}(L_{K}, L_{H}, r\mNumber{0}) \# O(n^\mNumber{2} r + n r^\mNumber{2}) \\
p\mNumber{0} = z\mNumber{0} \\
repeat until convergence: \\
w = matvec_{A,\tau}(p) \# O(n^\mNumber{2} r + q r) \\
\alpha = (r^{T} z) / (p^{T} w) \\
x = x + \alpha \ast p \\
