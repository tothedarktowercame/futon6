...
Let \(T^\mNumber{3}\) be the 3D unit torus. Let \(\mu\) be the \(\Phi^\mNumber{4}_\mNumber{3}\) measure on D\textquotesingle(\(T^\mNumber{3}\)).
...
The \(\Phi^\mNumber{4}_\mNumber{3}\) measure \(\mu\) on D\textquotesingle(\(T^\mNumber{3}\)) is the probability measure formally
written as:

\[dmu(\phi) = Z^{-\mNumber{1}} exp(-V(\phi)) dmu_\mNumber{0}(\phi)\]
...
\[V(\phi) = integral_{T^\mNumber{3}} (:\phi^\mNumber{4}: - C :\phi^\mNumber{2}:) dx\]
...
\(E_{\mu_\mNumber{0}}\){[}exp(-V){]} \textless{} infinity and hence \(\mu\) \textless\textless{} \(\mu_\mNumber{0}\) with strictly positive
density exp(-V(phi))/Z. Since exp(-V(phi)) \textgreater{} 0 a.s. (exponential is always
positive), the reverse absolute continuity \(\mu_\mNumber{0}\) \textless\textless{} \(\mu\) also holds.
...
\[H = H^\mNumber{1}(T^\mNumber{3}) (Sobolev space of order \mNumber{1})\]

Since \(\psi\) is smooth, \(\psi \in C^{infinity}(T^\mNumber{3})\) subset \(H^\mNumber{1}(T^\mNumber{3}),\) so \(\psi\) is in the
Cameron-Martin space.

\textbf{Cameron-Martin theorem:} For any \(h \in H\), the shifted Gaussian measure
\(T_{h}^*\) \(\mu_\mNumber{0}\) is equivalent to \(\mu_\mNumber{0},\) with Radon-Nikodym derivative:

\[dT_{h}^ \ast \mu_\mNumber{0} / dmu_\mNumber{0} (\phi) = exp(l_{h}(\phi) - ||h||_H^\mNumber{2} / \mNumber{2})\]

where \(l_{h}\) is the linear functional associated to h. In particular:

\[T_{\psi}^ \ast \mu_\mNumber{0} ~ \mu_\mNumber{0}\]
...
\[dT_{\psi}^ \ast \mu / dmu_\mNumber{0} (\phi) = Z^{-\mNumber{1}} exp(-V(\phi - \psi)) \ast exp(l_{\psi}(\phi) - ||\psi||_H^\mNumber{2}/\mNumber{2})\]

The shifted interaction V(phi - psi) expands (using Wick ordering relative to \(\mu_\mNumber{0}\)):

\[\begin{aligned}
V(\phi - \psi) = V(\phi) - \mNumber{4} int \psi :\phi^\mNumber{3}: dx + \mNumber{6} int \psi^\mNumber{2} :\phi^\mNumber{2}: dx \\
- \mNumber{4} int \psi^\mNumber{3} :\phi: dx + int \psi^\mNumber{4} dx \\
- C(int :\phi^\mNumber{2}: dx - \mNumber{2} int \psi :\phi: dx + int \psi^\mNumber{2} dx) + (renorm. corrections)
\end{aligned}\]
...
\item
  \(\psi \in C^{infinity}(T^\mNumber{3})\) (smooth)
\item
...
\[E_{\mu}[exp(t |int \psi :\phi^\mNumber{3}: dx|)] < infinity for |t| < t_\mNumber{0}\]

where \(t_\mNumber{0} > \mNumber{0}\) depends on \textbar\textbar psi\textbar\textbar{}\emph{\({C^\mNumber{0}}\) and the coupling constant.
...
in the Radon-Nikodym derivative is bounded by 4 \textbar\textbar psi\textbar\textbar{}}\({C^\mNumber{0}}\) \textbar int \(:\phi^\mNumber{3}:\) dx\textbar,
...
\[-log Z = inf_{u} E[V(\phi + int_\mNumber{0}^\mNumber{1} u_{s} ds) + \mNumber{1}/\mNumber{2} int_\mNumber{0}^\mNumber{1} ||u_{s}||^\mNumber{2} ds]\]
...
\item
  \(T_{\psi}^*\) \(\mu_\mNumber{0}\) \textasciitilde{} \(\mu_\mNumber{0}\) (Cameron-Martin theorem, since \(\psi \in H^\mNumber{1}\))
\item
