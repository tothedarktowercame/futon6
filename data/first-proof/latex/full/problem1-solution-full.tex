...
Let \(\psi\): \(T^\mNumber{3} \to R\) be a smooth nonzero function and \(T_{\psi}(u) = u \mBridgeOperator{+} \psi\) the
...
(\(m^\mNumber{2} \mBridgeOperator{-} \Delta\))\^{}\{-1\} on \(T^\mNumber{3}\), and the interaction is:

\[V(\phi) = \Integral_{T^\mNumber{3}} (:\phi^\mNumber{4}: \mBridgeOperator{-} C :\phi^\mNumber{2}:) \mathrm{d}x\]
...
\(E_{\mu_\mNumber{0}}[\exp(-V)] < \infty\) and hence \(\mu \ll \mu_\mNumber{0}\) with strictly positive
density exp(-V(\(\phi\)))/Z. Since exp(-V(\(\phi)) > \mNumber{0}\) a.s. (exponential is always
positive), the reverse absolute continuity \(\mu_\mNumber{0} \ll \mu\) also holds.
...
For the Gaussian free field \(\mu_\mNumber{0}\) on \(T^\mNumber{3}\) with covariance C \(= (m^{\mNumber{2}} \mBridgeOperator{-} \Delta)^{-\mNumber{1}},\)
...
\[\mathrm{d}T_{h}^{\mDualStar} \mu_\mNumber{0} / d\mu_\mNumber{0} (\phi) = \exp(l_{h}(\phi) \mBridgeOperator{-} \|h\|_{H^\mNumber{2}} / \mNumber{2})\]
...
\[\mathrm{d}T_{\psi}^{\mDualStar} \mu / d\mu_\mNumber{0} (\phi) = Z^{-\mNumber{1}} \exp(-V(\phi \mBridgeOperator{-} \psi)) \ast \exp(l_{\psi}(\phi) \mBridgeOperator{-} \|\psi\|_{H^\mNumber{2}}/\mNumber{2})\]

The shifted interaction V(\(\phi \mBridgeOperator{-} \psi\)) expands (using Wick ordering relative to \(\mu_\mNumber{0}\)):

\[\begin{aligned}
V(\phi - \psi) = V(\phi) \mBridgeOperator{-} \mNumber{4} \Integral \psi :\phi^\mNumber{3}: \mathrm{d}x + \mNumber{6} \Integral \psi^\mNumber{2} :\phi^\mNumber{2}: \mathrm{d}x \\
- \mNumber{4} \Integral \psi^\mNumber{3} :\phi: \mathrm{d}x + \Integral \psi^\mNumber{4} \mathrm{d}x \\
- C(\Integral :\phi^\mNumber{2}: \mathrm{d}x \mBridgeOperator{-} \mNumber{2} \Integral \psi :\phi: \mathrm{d}x + \Integral \psi^\mNumber{2} \mathrm{d}x) + (\text{renorm}. \text{corrections})
\end{aligned}\]
...
\[C \to C + \mNumber{6} \|\psi\|_{L^\mNumber{2}}^{\mNumber{2}} \ast (\text{log }N\text{ correction})\]
...
After renormalization, V(\(\phi \mBridgeOperator{-} \psi\)) - V(\(\phi\)) is a well-defined random variable
...
\[R(\phi) = \exp(-(V(\phi-\psi) - V(\phi)) + l_{\psi}(\phi) - \text{const})\]
...
\[E_{\mu}[\exp(t |\Integral \psi :\phi^\mNumber{3}: \mathrm{d}x|)] < \infty \text{for} |t| < t_\mNumber{0}\]
...
Therefore \(R \in L^\mNumber{1}(\mu)\) and 1/\(R \in L^\mNumber{1}(T_{\psi}^{\mDualStar} \mu),\) giving:

\[T_{\psi}^{\mDualStar} \mu ~ \mu (\text{equivalent measures})\]
...
\[-\log Z = \inf_{u} E[V(\phi + \Integral_\mNumber{0}^\mNumber{1} u_{s} \mathrm{d}s) + \mNumber{1}/\mNumber{2} \Integral_\mNumber{0}^\mNumber{1} ||u_{s}||^\mNumber{2} \mathrm{d}s]\]
...
\item
  V(\(\phi \mBridgeOperator{-} \psi\)) - V(\(\phi\)) is well-defined after renormalization
\item
...
\item
  M. Gubinelli, P. Imkeller, N. Perkowski, \texttt{Paracontrolled\ distributions\ and\ singular\ PDEs,} Forum Math. \(\Pi\) 3 (2015). {[}Proposition 6.3: explicit counterterm formula{]}
\end{itemize}
