...
(\(m^{\mNumber{2}} - \Delta)^{-\mNumber{1}}\) on \(T^\mNumber{3}\), and the interaction is:
...
For the Gaussian free field \(\mu_\mNumber{0}\) on \(T^\mNumber{3}\) with covariance C \(= (m^{\mNumber{2}} - \Delta)^{-\mNumber{1}},\)
...
\[\mathrm{d}T_{h}^{\mDualStar} \mu_\mNumber{0} / d\mu_\mNumber{0} (\phi) = \exp(l_{h}(\phi) - \|h\|_{H^\mNumber{2}} / \mNumber{2})\]
...
\[\mathrm{d}T_{\psi}^{\mDualStar} \mu / d\mu_\mNumber{0} (\phi) = Z^{-\mNumber{1}} \exp(-V(\phi - \psi)) \ast \exp(l_{\psi}(\phi) - \|\psi\|_{H^\mNumber{2}}/\mNumber{2})\]

The shifted interaction V(\(\phi - \psi)\) expands (using Wick ordering relative to \(\mu_\mNumber{0}\)):

\[\begin{aligned}
V(\phi - \psi) = V(\phi) - \mNumber{4} \Integral \psi :\phi^\mNumber{3}: \mathrm{d}x + \mNumber{6} \Integral \psi^\mNumber{2} :\phi^\mNumber{2}: \mathrm{d}x \\
- \mNumber{4} \Integral \psi^\mNumber{3} :\phi: \mathrm{d}x + \Integral \psi^\mNumber{4} \mathrm{d}x \\
- C(\Integral :\phi^\mNumber{2}: \mathrm{d}x - \mNumber{2} \Integral \psi :\phi: \mathrm{d}x + \Integral \psi^\mNumber{2} \mathrm{d}x) + (\text{renorm}. \text{corrections})
\end{aligned}\]
...
\[C \to C + \mNumber{6} \|\psi\|_{L^\mNumber{2}}^{\mNumber{2}} \ast (\log N \text{correction})\]
