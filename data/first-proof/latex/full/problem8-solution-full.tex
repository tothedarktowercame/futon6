...
\(R^\mNumber{4}.\) A \textbf{Lagrangian smoothing} of K is a Hamiltonian isotopy \(K_{t}\) of smooth
...
spanning property (\{\(e_\mNumber{1}\),...,\(e_\mNumber{4}\)\} spans \(R^\mNumber{4}\)) is automatic --- it follows from
...
theorem (edges) and the vanishing flux in simply-connected \(R^\mNumber{2}\) (vertices).

\hypertarget{solution}{%
\subsection{Solution}\label{solution}}

\hypertarget{1-setup-lagrangian-planes-in-r4}{%
\subsubsection{\texorpdfstring{1. Setup: Lagrangian planes in \(R^\mNumber{4}\)}{1. Setup: Lagrangian planes in R\^{}\textbackslash mNumber\{4\}}}\label{1-setup-lagrangian-planes-in-r4}}

Identify \(R^\mNumber{4} = C^\mNumber{2}\) with symplectic form \(\omega = dx_\mNumber{1}\) ∧ \(dy_\mNumber{1} \mBridgeOperator{+} dx_\mNumber{2}\) ∧ \(dy_\mNumber{2}.\)
...
\[L_{i} = span(e_{i-\mNumber{1},i}, e_{i,i+\mNumber{1}})\]

The Lagrangian condition omega\textbar\_\{\(L_{i}\)\} = 0 requires:

\[\omega(e_{i-\mNumber{1},i}, e_{i,i+\mNumber{1}}) = \mNumber{0} for each i (mod \mNumber{4})\]
...
\textbf{Theorem (v2).} The 4 edge vectors \(e_\mNumber{1} = e_{\mNumber{12}},\) \(e_\mNumber{2} = e_{\mNumber{23}},\) \(e_\mNumber{3} = e_{\mNumber{34}},\)
\(e_\mNumber{4} = e_{\mNumber{41}}\) satisfy \(\omega\)(\(e_{i}\), \(e_{i+\mNumber{1}}\)) = 0 for all i (mod 4). This forces
a symplectic direct sum decomposition:

\[R^\mNumber{4} = V_\mNumber{1} \oplus V_\mNumber{2}\]

where \(V_\mNumber{1}\) = span(\(e_\mNumber{1}\), \(e_\mNumber{3}\)) and \(V_\mNumber{2}\) = span(\(e_\mNumber{2}\), \(e_\mNumber{4}\)) are symplectic 2-planes
...
conditions \(\omega\)(\(e_{i}\), \(e_{i+\mNumber{1}}\)) = 0 kill 4 of the 6 independent entries,
leaving only:
...
1-dimensional edges), the 4 edge vectors \{\(e_\mNumber{1}\), \(e_\mNumber{2},\) \(e_\mNumber{3},\) \(e_\mNumber{4}\)\} span \(R^\mNumber{4}.\)

\emph{Proof.} Suppose for contradiction that the 4 edge vectors lie in a
3-dimensional subspace H ⊂ \(R^\mNumber{4}.\) Then all 4 faces \(L_{i}\) = span(\(e_{i-\mNumber{1},i}\),
\(e_{i,i+\mNumber{1}}\)) ⊂ H.

\textbf{Step 1.} The restricted form omega\textbar\_H has rank 2: since \(\omega\) is
non-degenerate on \(R^\mNumber{4},\) its restriction to a codimension-1 subspace has a
...
H/ell ≅ \(R^\mNumber{2}.\) But omega\textbar\_H descends to a non-degenerate 2-form on H/ell
(since ell = ker(omega\textbar\_H)), and a 2-plane in \(R^\mNumber{2}\) on which a
...
1-dimensional: dim(\(L_{i}\) ∩ \(L_{i+\mNumber{1}}\)) = 1, so \(L_{i}\) ∩ \(L_{i+\mNumber{1}} = span(e_{i}).\)

By Step 2, ell ⊂ \(L_{i}\) and ell ⊂ \(L_{i+\mNumber{1}},\) so ell ⊂ \(L_{i}\) ∩ \(L_{i+\mNumber{1}} = span(e_{i}).\) Since both ell and \(span(e_{i})\) are 1-dimensional subspaces and
one contains the other, ell = \(span(e_{i}).\)

Applying this to all four edges: \(span(e_\mNumber{1})\) = ell = \(span(e_\mNumber{2}) = span(e_\mNumber{3}) = span(e_\mNumber{4}).\) That is, all four edge vectors are proportional.

But then \(L_\mNumber{1}\) = span(\(e_\mNumber{4}\), \(e_\mNumber{1}\)) = \(span(e_\mNumber{1})\) is 1-dimensional, contradicting
...
This is block diagonal: (\(V_\mNumber{1}\), a) ⊕ (\(V_\mNumber{2}\), b), a symplectic direct sum. ∎

\textbf{Consequence:} Each Lagrangian face decomposes as a direct sum of lines:

\[\begin{aligned}
L_\mNumber{1} = span(e_\mNumber{4}) \oplus span(e_\mNumber{1}) \subset V_\mNumber{2} \oplus V_\mNumber{1} \\
L_\mNumber{2} = span(e_\mNumber{1}) \oplus span(e_\mNumber{2}) \subset V_\mNumber{1} \oplus V_\mNumber{2} \\
L_\mNumber{3} = span(e_\mNumber{2}) \oplus span(e_\mNumber{3}) \subset V_\mNumber{2} \oplus V_\mNumber{1} \\
L_\mNumber{4} = span(e_\mNumber{3}) \oplus span(e_\mNumber{4}) \subset V_\mNumber{1} \oplus V_\mNumber{2}
\end{aligned}\]

Each \(L_{i}\) takes one line from \(V_\mNumber{1}\) and one from \(V_\mNumber{2},\) which is automatically
Lagrangian in \(V_\mNumber{1}\) ⊕ \(V_\mNumber{2}\) (omega restricted to a line is zero).
...
The Maslov index of the vertex loop \(L_\mNumber{1}\) → \(L_\mNumber{2}\) → \(L_\mNumber{3}\) → \(L_\mNumber{4}\) → \(L_\mNumber{1} \in \Lambda(\mNumber{2})\) decomposes via the direct sum:

\[\mu = \mu_\mNumber{1} + \mu_\mNumber{2}\]

where \(\mu_{j}\) is the winding number of the component loop in \(V_{j}.\)

\textbf{In \(V_\mNumber{1}\) = span(\(e_\mNumber{1}\), \(e_\mNumber{3}\)):} The loop traces
\(span(e_\mNumber{1})\) → \(span(e_\mNumber{1})\) → \(span(e_\mNumber{3})\) → \(span(e_\mNumber{3})\) → \(span(e_\mNumber{1})\)

This is a back-and-forth path (\(e_\mNumber{1}\) → \(e_\mNumber{1}\) → \(e_\mNumber{3}\) → \(e_\mNumber{3}\) → \(e_\mNumber{1}\)), not a
winding loop. The winding number is 0.

\textbf{In \(V_\mNumber{2}\) = span(\(e_\mNumber{2}\), \(e_\mNumber{4}\)):} Similarly:
\(span(e_\mNumber{4})\) → \(span(e_\mNumber{2})\) → \(span(e_\mNumber{2})\) → \(span(e_\mNumber{4})\) → \(span(e_\mNumber{4})\)
...
But in (\(R^\mNumber{4}\), omega), the maximum isotropic dimension is 2 (= half the
dimension). Three independent isotropic vectors cannot exist.

\textbf{Therefore: a non-degenerate 3-face Lagrangian vertex is impossible in \(R^\mNumber{4}.\)}

The 4-face condition is precisely the right valence: it gives enough edges to
span \(R^\mNumber{4}\) while the cyclic \(\omega\)-orthogonality creates the symplectic direct
sum that forces Maslov index 0.

For 5 or more faces: the \(\omega\)-orthogonality conditions are over-determined
and generically have no solution with the edges spanning \(R^\mNumber{4}.\) The 4-face case
...
The key observation: the \(V_\mNumber{1}\) ⊕ \(V_\mNumber{2}\) decomposition (Section 3) gives K near
...
\(V_\mNumber{1}\) ⊕ \(V_\mNumber{2}:\) let \(V_\mNumber{1}\) have coordinates (x, y) with \(\omega_\mNumber{1}\) = dx ∧ dy, and \(V_\mNumber{2}\)
have coordinates (u, v) with \(\omega_\mNumber{2}\) = du ∧ dv (so \(\omega = \omega_\mNumber{1} \mBridgeOperator{+} \omega_\mNumber{2}\)).
...
(Here \(e_\mNumber{1},\) \(e_\mNumber{3}\) ∈ \(V_\mNumber{1}\) and \(e_\mNumber{2},\) \(e_\mNumber{4}\) ∈ \(V_\mNumber{2},\) consistent with the decomposition
in Section 3. This can always be arranged by a linear symplectomorphism.)

The 4 faces in these coordinates are:

\[\begin{aligned}
L_\mNumber{1} = span(e_\mNumber{4}, e_\mNumber{1}) = {y = \mNumber{0}, u = \mNumber{0}}, sector x > \mNumber{0}, v > \mNumber{0} \\
L_\mNumber{2} = span(e_\mNumber{1}, e_\mNumber{2}) = {y = \mNumber{0}, v = \mNumber{0}}, sector x > \mNumber{0}, u > \mNumber{0} \\
L_\mNumber{3} = span(e_\mNumber{2}, e_\mNumber{3}) = {x = \mNumber{0}, v = \mNumber{0}}, sector y > \mNumber{0}, u > \mNumber{0} \\
L_\mNumber{4} = span(e_\mNumber{3}, e_\mNumber{4}) = {x = \mNumber{0}, u = \mNumber{0}}, sector y > \mNumber{0}, v > \mNumber{0}
\end{aligned}\]
...
\textbf{Claim:} K ∩ B(v, r) = \(C_\mNumber{1}\) × \(C_\mNumber{2}\) (product of the two corners) for small r.

\emph{Verification:} A point (p, q) with p ∈ \(C_\mNumber{1}\) and q ∈ \(C_\mNumber{2}\) has p = (x, 0) or
(0, y) and q = (u, 0) or (0, v), giving 4 cases --- exactly the 4 faces
\(L_\mNumber{1},\) ..., \(L_\mNumber{4}\) listed above. ∎

\textbf{Smoothing.} Replace each corner \(C_{j}\) with a smooth curve \(C_{j}^{sm}\) ⊂ \(V_{j}\)
that agrees with \(C_{j}\) outside a ball of radius \(\delta\) around the origin:
...
Explicitly: parameterize \(C_\mNumber{1}^{sm}\) as \(\gamma_\mNumber{1}(t)\) for t ∈ R, where
\(\gamma_\mNumber{1}(t)\) = (t, 0) for t ≥ \(\delta\), \(\gamma_\mNumber{1}(t)\) = (0, -t) for t ≤ -delta,
and \(\gamma_\mNumber{1}\) is a smooth embedded curve for t ∈ {[}-delta, delta{]}. (Such a
...
\(K^{sm} = C_\mNumber{1}^{sm}\) × \(C_\mNumber{2}^{sm}\) is a product of smooth curves in the
...
Lagrangian submanifolds in (\(V_\mNumber{1}\), \(\omega_\mNumber{1}\)) × (\(V_\mNumber{2}\), \(\omega_\mNumber{2}\)) is Lagrangian
in (\(V_\mNumber{1}\) ⊕ \(V_\mNumber{2},\) \(\omega_\mNumber{1} \mBridgeOperator{+} \omega_\mNumber{2}\)): for tangent vectors (\(u_\mNumber{1}\), \(u_\mNumber{2}\)) and
(\(w_\mNumber{1}\), \(w_\mNumber{2}\)) to \(C_\mNumber{1}^{sm}\) × \(C_\mNumber{2}^{sm},\) we have
...
since \(u_\mNumber{1},\) \(w_\mNumber{1}\) are tangent to \(C_\mNumber{1}^{sm}\) (1-dimensional in \(V_\mNumber{1}\)) and \(u_\mNumber{2},\) \(w_\mNumber{2}\)
are tangent to \(C_\mNumber{2}^{sm}\) (1-dimensional in \(V_\mNumber{2}\)). ∎

\textbf{Smoothness:} \(K^{sm} = C_\mNumber{1}^{sm}\) × \(C_\mNumber{2}^{sm}\) is the image of the smooth
map (\(t_\mNumber{1}\), \(t_\mNumber{2}\)) → (\(\gamma_\mNumber{1}\)(\(t_\mNumber{1}\)), \(\gamma_\mNumber{2}\)(\(t_\mNumber{2}\))), which is a smooth immersion
...
surface, including at \(v = \gamma_\mNumber{1}(\mNumber{0})\) × \(\gamma_\mNumber{2}(\mNumber{0}).\) ∎

\textbf{Agreement:} \(K^{sm} = C_\mNumber{1}\) × \(C_\mNumber{2} = K\) outside the region where either
factor was modified (\textbar p\textbar{} ≤ \(\delta \in V_\mNumber{1}\) or \textbar q\textbar{} ≤ \(\delta \in V_\mNumber{2}\)), so \(K^{sm}\)
...
(The \(C^\mNumber{1}\) control issues noted in earlier drafts do not arise here: the
...
Such radii exist because the vertex set is finite and discrete in \(R^\mNumber{4}.\)
Within each \(B_{i},\) apply the product smoothing (Section 5a) using the
\(V_\mNumber{1}\) ⊕ \(V_\mNumber{2}\) decomposition from Section 3. This replaces K ∩ \(B_{i} = C_\mNumber{1}\) × \(C_\mNumber{2}\)
with the smooth Lagrangian surface \(C_\mNumber{1}^{sm}\) × \(C_\mNumber{2}^{sm}.\)
...
smoothing region shrinks and \(K_{t}\) → \(K = K_\mNumber{0} \in C^\mNumber{0},\) giving the topological
...
1-parameter family of Lagrangian graphs y = grad \(S_{t}(x) \in T^ \ast R^\mNumber{2}.\) By
...
manifold (here \(T^ \ast R^\mNumber{2}\) with \(\lambda = y\) dx) is generated by a Hamiltonian:
...
\(K_{t}^{vertex} = C_\mNumber{1}^{sm}(t)\) × \(C_\mNumber{2}^{sm}(t) \in V_\mNumber{1}\) × \(V_\mNumber{2}.\) Each \(C_{j}^{sm}(t)\)
is a smooth 1-parameter family of curves in (\(V_{j}\), \(\omega_{j}\)) ≅ (\(R^\mNumber{2}\), dx∧dy).
...
because \(V_{j} = R^\mNumber{2}\) is simply connected, so the flux homomorphism
Flux: \(\pi_\mNumber{1}(Symp)\) → \(H^\mNumber{1}\)(L; R) is trivial (no non-Hamiltonian symplectic
...
in \(R^\mNumber{2}\) --- the smoothed corner --- hence contractible). The primitive
is the Hamiltonian \(H_{j}(t).\) The product isotopy on \(V_\mNumber{1}\) × \(V_\mNumber{2}\) is Hamiltonian
...
\tightlist
\item
  The 4-face + Lagrangian condition forces a symplectic direct sum \(R^\mNumber{4} = V_\mNumber{1}\) ⊕ \(V_\mNumber{2}\)
...
\item
  Near each vertex, \(K = C_\mNumber{1}\) × \(C_\mNumber{2}\) (product of corners). Smoothing each corner
  gives \(K^{sm} = C_\mNumber{1}^{sm}\) × \(C_\mNumber{2}^{sm},\) a smooth Lagrangian surface (product of
...
\item
  ...
  they\textquotesingle re impossible (3 isotropic vectors can\textquotesingle t span 3D in \(R^\mNumber{4}\)).
\item
