...
By Step 2, ell ⊂ \(L_{i}\) and ell ⊂ \(L_{i+1},\) so ell ⊂ \(L_{i}\) ∩ \(L_{i+1}\) =
\(span(e_{i}).\) Since both ell and \(span(e_{i})\) are 1-dimensional subspaces and
...
the original polyhedral K outside a neighborhood of \(v_{i}\) of radius \(\delta\) \textless{} \(r_{i}.\)
