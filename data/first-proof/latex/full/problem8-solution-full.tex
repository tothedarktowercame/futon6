...
Identify \(\mathbb{R}^{\mNumber{4}} = \mathbb{C}^{\mNumber{2}}\) with symplectic form \(\omega = \mathrm{d}x_{\mNumber{1}} \wedge \mathrm{d}y_{\mNumber{1}} \mBridgeOperator{+} \mathrm{d}x_{\mNumber{2}} \wedge \mathrm{d}y_{\mNumber{2}}.\)
A Lagrangian plane is a 2-dimensional subspace L where \(\omega|_{L} = \mNumber{0}\). The space
...
\[\omega(e_{i-\mNumber{1},i}, e_{i,i+\mNumber{1}}) = \mNumber{0} \text{for} \text{each} i (\mOpName{mod} \mNumber{4})\]
...
\[\begin{aligned}
C_\mNumber{1}^{sm}: \text{smooth} \text{curve} \in V_\mNumber{1}, = {(x, \mNumber{0})} \text{for} x > \delta, = {(\mNumber{0}, y)} \text{for} y > \delta, \\
\text{smooth} \text{through} \text{the} \text{origin} (e.g., \text{the} \text{curve} (\cos \theta, \sin \theta) \\
\text{reparameterized} \text{to} \text{match} \text{the} \text{axes} \text{outside} \text{the} \text{transition}) \\
...
the 1-form \(i_{d/\mathrm{d}t}(\omega)|\)\emph{\(\{K_{t}\}\) is exact (it equals d(\(S_{t}\)\textbar{}}\{\(K_{t}\)\})),
