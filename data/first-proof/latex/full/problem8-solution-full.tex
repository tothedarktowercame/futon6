...
A \textbf{polyhedral Lagrangian surface} \(K \in R^{\mNumber{4}}\) is a finite polyhedral complex all
...
\([\mNumber{0},\mNumber{1}]\), with \(K_\mNumber{0} = K\).
...
spanning property (\(\{e_{\mNumber{1}}, \ldots, e_{\mNumber{4}}\}\) spans \(\mathbb{R}^{\mNumber{4}}\)) is automatic --- it follows from
...
Identify \(\mathbb{R}^{\mNumber{4}} = \mathbb{C}^{\mNumber{2}}\) with symplectic form \(\omega = \mathrm{d}x_{\mNumber{1}} \wedge \mathrm{d}y_{\mNumber{1}} \mBridgeOperator{+} \mathrm{d}x_{\mNumber{2}} \wedge \mathrm{d}y_{\mNumber{2}}. A\) Lagrangian plane is a 2-dimensional subspace L where \(\omega|_{L} = \mNumber{0}\). The space
...
The Lagrangian condition \(\omega|_{L_{i}} = \mNumber{0}\) requires:

\[\omega(e_{i-\mNumber{1},i}, e_{i,i+\mNumber{1}}) = \mNumber{0} \text{for each} i (\mOpName{mod} \mNumber{4})\]
...
\[\mathbb{R}^{\mNumber{4}} = V_\mNumber{1} \oplus V_\mNumber{2}\]
...
with \(\omega|_{V_\mNumber{1} \times V_\mNumber{2}} = \mNumber{0}\).
...
non-degenerate 2-form vanishes must be \(\{\mNumber{0}\}\). Contradiction. So ell \(\subset\) L.
...
The Maslov index of the vertex loop \(L_\mNumber{1} \to L_\mNumber{2} \to L_\mNumber{3} \to L_\mNumber{4} \to L_\mNumber{1} \in \Lambda\)(2) decomposes via the direct sum:
...
\(\mOpName{span}(e_\mNumber{1}) \to \mOpName{span}(e_\mNumber{1}) \to \mOpName{span}(e_\mNumber{3}) \to \mOpName{span}(e_\mNumber{3}) \to \mOpName{span}(e_\mNumber{1})\)
...
\(\mOpName{span}(e_\mNumber{4}) \to \mOpName{span}(e_\mNumber{2}) \to \mOpName{span}(e_\mNumber{2}) \to \mOpName{span}(e_\mNumber{4}) \to \mOpName{span}(e_\mNumber{4})\)
...
We do not claim a general obstruction for valence \(\ge\) 5 here; the proof
...
\[\begin{aligned}
L_\mNumber{1} = \mOpName{span}(e_\mNumber{4}, e_\mNumber{1}) = {y = \mNumber{0}, u = \mNumber{0}}, \text{sector} x > \mNumber{0}, v > \mNumber{0} \\
L_\mNumber{2} = \mOpName{span}(e_\mNumber{1}, e_\mNumber{2}) = {y = \mNumber{0}, v = \mNumber{0}}, \text{sector} x > \mNumber{0}, u > \mNumber{0} \\
L_\mNumber{3} = \mOpName{span}(e_\mNumber{2}, e_\mNumber{3}) = {x = \mNumber{0}, v = \mNumber{0}}, \text{sector} y > \mNumber{0}, u > \mNumber{0} \\
L_\mNumber{4} = \mOpName{span}(e_\mNumber{3}, e_\mNumber{4}) = {x = \mNumber{0}, u = \mNumber{0}}, \text{sector} y > \mNumber{0}, v > \mNumber{0}
\end{aligned}\]

Define the ``corner'' curves in each factor:

\[\begin{aligned}
C_\mNumber{1} = {(x, \mNumber{0}) : x \ge \mNumber{0}} ∪ {(\mNumber{0}, y) : y \ge \mNumber{0}} \subset V_\mNumber{1} (\text{positive} x \text{and} y \text{axes}) \\
C_\mNumber{2} = {(u, \mNumber{0}) : u \ge \mNumber{0}} ∪ {(\mNumber{0}, v) : v \ge \mNumber{0}} \subset V_\mNumber{2} (\text{positive} u \text{and} v \text{axes})
\end{aligned}\]

\textbf{Claim:} K ∩ B(v, r) \(= C_{\mNumber{1}} \times C_{\mNumber{2}}\) (product of the two corners) for small r.

\emph{Verification:} A point \((p, q)\) with \(p \in C_\mNumber{1}\) and \(q \in C_\mNumber{2}\) has \(p = (x, \mNumber{0})\) or
\((\mNumber{0}, y)\) and \(q = (u, \mNumber{0})\) or \((\mNumber{0}, v)\), giving 4 cases --- exactly the 4 faces
\(L_{\mNumber{1}}, ..., L_{\mNumber{4}}\) listed above. ∎

\textbf{Smoothing.} Replace each corner \(C_{j}\) with a smooth curve \(C_{j}^{sm} \subset V_{j}\)
that agrees with \(C_{j}\) outside a ball of radius \(\delta\) around the origin:

\[\begin{aligned}
C_\mNumber{1}^{sm}: \text{smooth curve} \in V_\mNumber{1}, = {(x, \mNumber{0})} \text{for} x > \delta, = {(\mNumber{0}, y)} \text{for} y > \delta, \\
\text{smooth through the origin} (e.g., \text{the curve} (\cos \theta, \sin \theta) \\
\text{reparameterized to match the axes outside the transition}) \\
C_\mNumber{2}^{sm}: \text{analogous} \in V_\mNumber{2}
\end{aligned}\]

Explicitly: parameterize \(C_\mNumber{1}^{sm}\) as \(\gamma_\mNumber{1}(t)\) for \(t \in R\), where
\(\gamma_\mNumber{1}(t) = (t, \mNumber{0})\) for \(t \ge \delta, \gamma_{\mNumber{1}}(t) = (\mNumber{0},\) -t) for t \(\le -\delta,\)
...
(the tangent vectors d \(\gamma_\mNumber{1}/\mathrm{d}t_\mNumber{1}\) and d \(\gamma_\mNumber{2}/\mathrm{d}t_\mNumber{2}\) are nonzero and lie
...
factor was modified (\textbar p\textbar{} \(\le \delta \in V_{\mNumber{1}}\) or \(|q| \le \delta \in V_\mNumber{2}\)), so \(K^{sm}\)
...
Define \(S_{\epsilon}(x) = \chi(x_{\mNumber{1}}/\epsilon) S_{\mNumber{1}}(x) + (\mNumber{1} \mBridgeOperator{-} \chi(x_{\mNumber{1}}\)/eps)) \(S_\mNumber{2}(x)\) for a
smooth cutoff \(\chi\). The graph y \(=\) grad \(S_{\epsilon}\) is Lagrangian (graph of the
exact 1-form \(\mathrm{d}S_{\epsilon}\)). For any fixed \(\epsilon > \mNumber{0}\), this gives a smooth Lagrangian
...
on the annular region \(\delta < |x| < r_{i})\), they glue to a globally smooth
Lagrangian surface.

\textbf{Step 7c: Global Hamiltonian isotopy.} Each smoothing is parameterized by
a width parameter t (\(\delta\) for vertices, eps for edges). As \(t \to \mNumber{0}\), the
smoothing region shrinks and \(K_{t} \to K = K_{\mNumber{0}} \in C^{\mNumber{0}},\) giving the topological
...
\(S_{t}(x) = \chi(x_{\mNumber{1}}/t) S_{\mNumber{1}}(x) + (\mNumber{1} \mBridgeOperator{-} \chi(x_{\mNumber{1}}\)/t)) \(S_\mNumber{2}(x)\) defines a smooth
...
the 1-form \(\iota_{d/\mathrm{d}t}(\omega)|_{K_{t}}\) is exact (it equals \(d(S_{t}|_{K_{t}})\)),
so the isotopy is Hamiltonian with generating function \(H_{t} = \mathrm{d}S_{t}/\mathrm{d}t\)
evaluated on the Lagrangian.

\textbf{Vertex smoothings are Hamiltonian.} The vertex smoothing is a product:
\(K_{t}^{\text{vertex}} = C_\mNumber{1}^{sm}(t) \times C_\mNumber{2}^{sm}(t) \in V_\mNumber{1} \times V_\mNumber{2}\). Each \(C_{j}^{sm}(t)\)
...
Flux: \(\pi_\mNumber{1}(\mathup{Symp}) \to H^\mNumber{1}\)(L; R) is trivial (no non-Hamiltonian symplectic
...
group \(\mathup{Ham}_{c}\)(M, \(\omega\)) of compactly supported Hamiltonian diffeomorphisms
...
\(V \mBridgeOperator{-} E \mBridgeOperator{+} F = \chi\)(K), with 4F \(=\) 2E and 4V \(=\) 2E, giving \(\chi = \mNumber{0}\).
...
\tightlist
\item
  The 4-face + Lagrangian condition forces a symplectic direct sum \(\mathbb{R}^{\mNumber{4}} = V_{\mNumber{1}} \oplus V_{\mNumber{2}}\)
...
\tightlist
\item
  \textbf{v1 claimed} Maslov index vanishes by \texttt{alternating\ orientations\ relative\ to\ J.} \textbf{Replaced} with the precise symplectic direct sum argument: the
...
\item
  ...
  quadruples (55\% have \(\mu = \mNumber{0}\)) confirms the constraint is essential.
\end{itemize}
