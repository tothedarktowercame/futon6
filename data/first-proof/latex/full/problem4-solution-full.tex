...
\[E_{U}[char. poly. of A + \mathup{UBU} * ] = p_{A} \boxplus_{n} p_{B}\]
...
to log \textbar det(xI - A)\textbar{} is conceptual: away from roots, \(F_{A}''(x) = -\\sum_{i}\) 1/(x - \(\lambda_{i}\))\^{}2,
...
\[\Phi_\mNumber{3}(p) \ast disc(p) = \mNumber{18} \ast a_\mNumber{2}^\mNumber{2} (\text{EXACT})\]
...
\[u^\mNumber{2}/s^\mNumber{2} + v^\mNumber{2}/t^\mNumber{2} \ge (u+v)^\mNumber{2}/(s^\mNumber{2}+t^\mNumber{2}) [\text{Titu}'s lemma]\]
...
(b) \(\boxplus_\mNumber{4}\) has a cross-term even for centered polynomials: \(c_\mNumber{4} = a_\mNumber{4}\) + (1/6)\emph{\(a_\mNumber{2}\)}\(b_\mNumber{2} \mBridgeOperator{+} b_\mNumber{4}\).
Unlike n=3, centering does NOT reduce \(\boxplus_\mNumber{4}\) to plain coefficient addition.
...
3 & \textbf{PROVED} (strict ineq.) & \(\Phi_\mNumber{3} \ast disc=\mNumber{18}a_\mNumber{2}^\mNumber{2}\) identity + Titu\textquotesingle s lemma \\
...
\item
  ...
  fails \textasciitilde40\% (n=3 centered excluded: 0\% because \(\boxplus_\mNumber{3}\) = addition there)
\end{enumerate}
...
This fails at n=4 (the product depends on all coefficients). The \(\boxplus_\mNumber{4}\)
...
Ratio = LHS/RHS = \([\mNumber{1}/\Phi_{n}(p\boxplus q)] / [\mNumber{1}/\Phi_{n}(p) \mBridgeOperator{+} \mNumber{1}/\Phi_{n}(q)]\); ratio \textgreater= 1
