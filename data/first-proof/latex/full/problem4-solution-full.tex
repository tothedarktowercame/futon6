...
Condition (3) is verified numerically for $n \ge 4$ (0 violations in 100 tests each)
but fails at $n = 3$ (3/100 violations). The Stam inequality for $n = 3$ is already
proved by direct algebraic methods (Section 5a), so the gap is formal only.

For $n \ge 4$, a proof of the log-discriminant superadditivity (3) would complete
the Stam inequality proof via the de Bruijn route. The remaining step is a
``score decomposition'' or ``entropy projection'' argument --- the finite analog
of the free Cauchy-Schwarz on conjugate variables in Voiculescu (1998).
