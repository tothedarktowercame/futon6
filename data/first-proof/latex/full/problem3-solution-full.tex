...
Let \(\lambda = (\lambda_{\mNumber{1}} > ... > \lambda_{n} \ge \mNumber{0})\) be a restricted partition with
...
\[\pi(\mu) = F^{\prime}_{\mu}(x_\mNumber{1}, ..., x_{n}; q=\mNumber{1}, t) / P^{\prime}_{\lambda}(x_\mNumber{1}, ..., x_{n}; q=\mNumber{1}, t)\]
...
with content \(\lambda\) and parameters \(x_{\mNumber{1}}, ..., x_{n}.\) This is a concrete continuous-
time Markov chain defined directly from local species comparisons and ringing
rates \(\mNumber{1}/x_{i}\) (no use of \(F^{\prime}_{\mu}\) in the transition rule). A theorem of
Ayyer-Martin-Williams identifies its stationary distribution as

\[\pi(\mu) = F_{\mu}(x; \mNumber{1}, t) / P_{\lambda}(x; \mNumber{1}, t),\]

which is the ratio required in the problem statement (same \(q = \mNumber{1}\) ASEP/Macdonald
...
\[S_{n}(\lambda) = \{\text{all permutations of the parts of } \lambda\}.\]

Because \(\lambda\) has distinct parts, \(|S_{n}(\lambda)| = n!\).
...
Fix parameters \(x_{\mNumber{1}}, ..., x_{n} > \mNumber{0}\) and t in {[}0,1). At each site j, an exponential
clock of rate \(\mNumber{1}/x_{j}\) rings.

If site j is a vacancy (species 0), nothing happens. If site j has species
\(r_\mNumber{0} > \mNumber{0}\), that particle becomes active. Let m be the number of particles in the
current configuration with species strictly less than \(r_\mNumber{0}\) (including vacancies).
Moving clockwise, the active particle chooses the k-th weaker particle with
probability

\[t^{(k-\mNumber{1})} / [m]_t, \text{where} [m]_t = \mNumber{1} + t + ... + t^{(m-\mNumber{1})},\]
...
\tightlist
\item
  Transition rates are defined from site rates \(\mNumber{1}/x_{i}\), species inequalities, and
...
\(x_{\mNumber{1}}, ..., x_{n} > \mNumber{0},\) and \(\mNumber{0} \le t < \mNumber{1}\), the stationary probability of
\(\eta \in S_{n}(\lambda)\) is

\[\pi(\eta) = F_{\eta}(x; \mNumber{1}, t) / P_{\lambda}(x; \mNumber{1}, t),\]

where \(F_{\eta}\) are ASEP polynomials at \(q = \mNumber{1}\) and \(P_{\lambda}\)(x; 1, t) \(= \sum_{\nu \in S_{n}(\lambda)} F_{\nu}\)(x; 1, t) is the partition function
(ensuring \(\pi\) sums to 1). Positivity: \(F_{\eta}(x; \mNumber{1}, t) > \mNumber{0}\) for \(x_{i} > \mNumber{0}, \mNumber{0} \le t < \mNumber{1}\) is established as part of AMW Theorem 1.1, which shows that
...
\[\begin{aligned}
P_{\lambda}(x; \mNumber{1}, t) = \sum_{\eta \in S_{n}(\lambda)} F_{\eta}(x; \mNumber{1}, t) \\
P^{\prime}_{\lambda}(x; \mNumber{1}, t) = \sum_{\eta \in S_{n}(\lambda)} F^{\prime}_{\eta}(x; \mNumber{1}, t)
\end{aligned}\]

Only ratio-invariance is needed: if conventions differ by an \(\eta\)-independent
normalization (for fixed \(\lambda\)), then \(F^{\prime}_{\mu} / P^{\prime}_{\lambda} = F_{\mu} / P_{\lambda}.\)
Concretely, if \(F^{\prime}_{\eta} = \alpha \ast F_{\eta}\) for some constant \(\alpha\) independent
of \(\eta\), then \(P^{\prime}_{\lambda} = \alpha \ast P_{\lambda}\) and:
...
normalizations and verify their equivalence at \(q = \mNumber{1}\)). The normalization is
fixed by the leading-term convention, which is uniform across the state
space \(S_{n}(\lambda).\) Concretely, at \(q = \mNumber{1}\) both families specialize to the same
t-weight formula, so \(\alpha = \mNumber{1}\) and \(F^{\prime}_{\eta} = F_{\eta}\).

\textbf{Verification for \(n = \mNumber{2}\).} Take \(\lambda = (a, \mNumber{0})\). Both conventions give
\(F_{(a,\mNumber{0})}(x_{\mNumber{1}}, x_{\mNumber{2}}\); 1, t) \(= x_{\mNumber{1}}\) and \(F_{(\mNumber{0},a)}(x_{\mNumber{1}}, x_{\mNumber{2}}\); 1, t) \(= x_{\mNumber{2}}\)
(the single-species case reduces to site weights). The ratio
\(F^{\prime}_{\eta} / P^{\prime}_{\lambda} = F_{\eta} / P_{\lambda} = x_{i} / (x_{\mNumber{1}} \mBridgeOperator{+} x_{\mNumber{2}})\) in both conventions.

\hypertarget{6-sanity-check-n2-reduction}{%
\subsubsection{\texorpdfstring{6. Sanity check: \(n = \mNumber{2}\) reduction}{6. Sanity check: n = \textbackslash mNumber\{2\} reduction}}\label{6-sanity-check-n2-reduction}}

Take \(\lambda = (a, \mNumber{0})\), state space \(\{(a, \mNumber{0}), (\mNumber{0}, a)\}\). There is one non-vacancy and one
vacancy. Bells ring at rates \(\mNumber{1}/x_\mNumber{1}\) and \(\mNumber{1}/x_\mNumber{2}\).

\begin{itemize}
\tightlist
\item
  \((a, \mNumber{0}) \to (\mNumber{0}, a)\) at rate \(\mNumber{1}/x_\mNumber{1}\)
\item
  \((\mNumber{0}, a) \to (a, \mNumber{0})\) at rate \(\mNumber{1}/x_\mNumber{2}\)
\end{itemize}

So the stationary distribution is

\[\pi(a,\mNumber{0}) = x_\mNumber{1}/(x_\mNumber{1}+x_\mNumber{2}), \pi(\mNumber{0},a) = x_\mNumber{2}/(x_\mNumber{1}+x_\mNumber{2}),\]
...
For \(x_{i} > \mNumber{0}\) and \(\mNumber{0} \le t < \mNumber{1}\), the inhomogeneous multispecies t-PushTASEP on
\(S_{n}(\lambda)\) is:
...
\textbf{Irreducibility proof (via vacancy transport).} Since \(\lambda_{n} = \mNumber{0}\), every
...
the vacancy at site \(j + \mNumber{1}\) (cyclically), and the clock at j rings, one possible
...
probability \(\mNumber{1}/[m]_t > \mNumber{0}\) (the \(k = \mNumber{1}\) term in the t-geometric distribution, where
\(m \ge \mNumber{1}\) counts weaker particles). The particle moves to the vacancy\textquotesingle s position,
