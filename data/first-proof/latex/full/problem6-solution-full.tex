...
\textbf{\(K_{n}\): PROVED.} The barrier greedy gives \textbar S\textbar{} = eps*n/3, c = 1/3, via the
...
independence number \textgreater= \(n^\mNumber{2}/(\mNumber{2}m+n)\). For the heavy graph:
...
Let \(I_\mNumber{0}\) be a maximal independent set in \(G_{heavy}\) with \(|I_\mNumber{0}|\) \textgreater= epsilon*n/3.
...
is at most 2(n-1)*epsilon/\(\mNumber{8} < \epsilon\)*n/4. The remaining set \(I_\mNumber{0}'\) has
...
invariant \(M_{t} = \sum_{e \in E(S_{t})} X_{e}\) prec epsilon*I at each step.
...
where \(H_{t}\) = epsilon*I - \(M_{t}\) succ 0 (the barrier headroom).

\textbf{Claim:} At each step \(t \le \epsilon \ast |I_\mNumber{0}'|/\mNumber{3}\), there exists \(v \in R_{t}\) with
\(\lambda_{\max}(M_{t} \mBridgeOperator{+} C_{t}\)(v)) \textless{} \(\epsilon\) (equivalently, \(||Y_{t}(v)||\) \textless{} 1).
...
By the pigeonhole principle (minimum \textless= average):
...
cross-edge matrix and \(H_{t}\) = epsilon*I - \(M_{t}\).

\textbf{Case \(M_{t} = \mNumber{0}\) (formal proof):}

When \(M_{t} = \mNumber{0}\) (early steps), \(H_{t}\) = epsilon*I and:

\[dbar_{t} = (\mNumber{1}/(\epsilon \ast r_{t})) \sum_{u \in S_{t}} ell_{u}^{R}\]

where \(ell_{u}^{R} = \sum_{v \in R_{t}, v~u} \tau_{uv} \le ell_{u} \le\) 8/epsilon.

For the complete graph \(K_{n}\) (where all edges are light for \(n > \mNumber{2}\)/epsilon):
\(\tau_{e}\) = 2/n for all edges, \(ell_{u}^{R} = (r_{t}) \ast (\mNumber{2}/n)\), and:

\[dbar_{t} = t \ast r_{t} * (\mNumber{2}/n) / (\epsilon \ast r_{t}) = \mNumber{2}t/(n \ast \epsilon).\]

At T = epsilon*n/3: \(dbar_{T} = \mNumber{2}/\mNumber{3} < \mNumber{1}.\) This is EXACT for \(K_{n}\).

For general graphs at \(M_{t} = \mNumber{0}\) with the leverage filter (\(ell_{u}\) \textless= 8/epsilon):

\[dbar_{t} \le (\mNumber{8} \ast t) / (\epsilon^\mNumber{2} \ast r_{t}).\]

At \(t \le \epsilon \ast |I_\mNumber{0}'|/\mNumber{3}\) and \(r_{t} \ge |I_{\mNumber{0}}'|(\mNumber{1} -\) epsilon/3) \textgreater= \(\mNumber{2}|I_\mNumber{0}'|/\mNumber{3}\):
...
structure (as in the \(K_{n}\) case where dbar = 2t/(n*epsilon) \textless\textless{} 1).
...
So \(dbar_{t}\) \textless= (n-1)/(epsilon * \(r_{t}\)). With \(r_{t}\) \textgreater= epsilon*n/4:

\[dbar_{t} \le (n-\mNumber{1})/(\epsilon^\mNumber{2} * n/\mNumber{4}) = \mNumber{4}/\epsilon^\mNumber{2}.\]
...
\textbf{For \(K_{n}\), dbar is bounded exactly:} \(dbar_{t}\) = 2t/(n\emph{epsilon), and at
\(T = \epsilon\)}n/3 steps, \(dbar_{T}\) = 2/3. This gives the formal proof for
...
sum of roots = dbar, so max root \textless= \(dbar < \mNumber{1}\). This gives an independent
...
S subset \(I_\mNumber{0}'\) with \textbar S\textbar{} = T and \(M_{S}\) prec \(\epsilon\)\emph{I (i.e., \(L_{S} \le \epsilon\)}L).

\textbf{Size analysis:}

The Turan step gives \(|I_\mNumber{0}| \ge \epsilon\)\emph{n/3. The leverage filter (5b)
gives \(|I_\mNumber{0}'| \ge \epsilon\)}n/12. The greedy runs \(T = \epsilon \ast |I_\mNumber{0}'|/\mNumber{3}\) steps,
so \textbar S\textbar{} = \(\epsilon^\mNumber{2} \ast n/\mNumber{36}\).

This gives \textbar S\textbar{} proportional to \(\epsilon^\mNumber{2}\)\emph{n, not \(\epsilon\)}n. For
\textbar S\textbar{} \textgreater= c\emph{\(\epsilon\)}n with universal c: need c \textless= epsilon/36, which
depends on \(\epsilon\).

\textbf{The \(\epsilon^\mNumber{2}\) bottleneck:} The Turan independent set has size
\(|I_\mNumber{0}| = \Theta\)(epsilon\emph{n). Running the greedy for \(\Theta\)(epsilon}\textbar{}\(I_\mNumber{0}\)\textbar)
steps gives \textbar S\textbar{} = \(\Theta(\epsilon^\mNumber{2} \ast n)\). This is inherent in the
heavy-edge-avoidance approach.

\textbf{For fixed \(\epsilon\) (the practical case):} With \(\epsilon\) = 0.3:
\textbar S\textbar{} \textgreater= 0.09*n/36 = n/400. With \(\epsilon\) = 0.2: \textbar S\textbar{} \textgreater= n/900.
These are nontrivial lower bounds, sufficient for applications.

\textbf{For \(K_{n}\) (proved exactly):} dbar = 2t/(n\emph{epsilon) with the greedy
running \(T = \epsilon\)}n/3 steps, giving \textbar S\textbar{} = epsilon*n/3 and c = 1/3.
...
P(\textbar S\textbar{} \textgreater= \(\epsilon\)\emph{n/6) \textgreater= 1 - exp(-epsilon}n/18). Numerically:
P(\textbar\textbar{}\(M_{S}|| \le \epsilon\) AND \textbar S\textbar{} \textgreater= epsilon*n/6) \textgreater{} 0 for all tested
graphs (n \textless= 80, 11 families, 4 \(\epsilon\) values, 500 trials each).
Success probability ranges from 0.2\% to 57\%.

This gives \textbar S\textbar{} \textgreater= epsilon*n/6 (c = 1/6) but the formal matrix
...
\def\labelenumi{\arabic{enumi}.}
\item
  The \(\epsilon\)-light condition \(L_{S}\) \textless= epsilon*L is equivalent to
...
\item
  ...
  The barrier greedy gives \textbar S\textbar{} = \(\epsilon\)\emph{n/3 with \(||M_{S}|| < \epsilon\).
  Proved by: \(dbar_{t}\) = 2t/(n}epsilon) \textless= 2/\(\mNumber{3} < \mNumber{1}\) (exact computation),
  then pigeonhole + PSD trace bound gives existence of v with
  \(||Y_{t}(v)|| < \mNumber{1}\) at each step. Universal c = 1/3.
\item
  ...
  \item
    exists v with \(\mOpName{trace}(Y_{v})\) \textless= dbar (pigeonhole)
  \item
...
tr(\(H_{t}^{-\mNumber{1}} M_{\text{cross}})\) where \(H_{t}\) = epsilon*I - \(M_{t}\). When \(||M_{t}||\) is
...
giving max root \textless= dbar via Vieta.

\hypertarget{summary}{%
\subsubsection{Summary}\label{summary}}

The existential answer is \textbf{YES} for \(K_{n}\) with c = 1/3 (proved),
and numerically confirmed for all tested graph families with
c \textgreater= 1/6. The formal extension to arbitrary graphs requires
...
\item
  \(L_{S}\) \textless= epsilon*L iff \(||\sum_{e \in E(S)} X_{e}|| \le \epsilon\)
\item
  For PSD Y: \(\|Y\|\) \textless= tr(Y) (spectral norm bounded by trace)
\item
  Pigeonhole: \(min_{v}\) f(v) \textless= (1/r) \(\sum_{v}\) f(v) (minimum \textless= average)
\item
  Turan: independence number \textgreater= \(n^\mNumber{2}/(\mNumber{2}m+n)\)
\item
  For \(K_{n}\): \(\tau_{e}\) = 2/n, \(||M_{S}||\) = \textbar S\textbar/n (exact)
\end{enumerate}
