...
For S subseteq V, define the induced-subgraph Laplacian embedded to \(R^{V \times V}\):
...
\(\epsilon \in (\mNumber{0}, \mNumber{1})\), there exists S with \(|S| \ge c \ast \epsilon \ast n\) and \(L_{S} \le \epsilon\) L?

\hypertarget{status-of-this-writeup}{%
\subsection{Status of this writeup}\label{status-of-this-writeup}}

\textbf{\(K_{n}\): PROVED.} The barrier greedy gives \(|S| = \epsilon \ast n/\mNumber{3}, c = \mNumber{1}/\mNumber{3},\) via the
...
prevents closure via Markov alone. See the attack-path analysis note.\footnote{\url{https://github.com/tothedarktowercame/futon6/blob/master/data/first-proof/problem6-gpl-h-attack-paths.md}}
...
\item
  ...
  \(E[|S|] = pn, Pr[|S| < pn/\mNumber{2}] \le \exp(-pn/\mNumber{8}) (Chernoff).\)
\item
...
\[E[\epsilon L \mBridgeOperator{-} L_{S}] = (\epsilon \mBridgeOperator{-} p^\mNumber{2})L.\]
...
\sum_{e} \tau_{e} = n \mBridgeOperator{-} k
\end{aligned}\]
...
Since \(\sum_{v} ell_{v} = \mNumber{2} \ast \sum_{e \text{internal to} I_{\mNumber{0}}} \tau_{e} \le\) 2(n-1),
by Markov:
...
\[\begin{aligned}
|I_\mNumber{0}'| \ge |I_\mNumber{0}| \mBridgeOperator{-} \epsilon \ast n/\mNumber{4} \ge \epsilon \ast n/\mNumber{3} - \epsilon \ast n/\mNumber{4} \\
...
where \(H_{t} = \epsilon \ast I \mBridgeOperator{-} M_{t}\) succ 0 (the barrier headroom).
...
(Proof: \(\|Y\| = \lambda_{\max}(Y)\), and \(\mOpName{tr}(Y) = \sum_{i} \lambda_{i} \ge \lambda_{\max}\)
...
cross-edge matrix and \(H_{t} = \epsilon \ast I \mBridgeOperator{-} M_{t}\).
...
\(\tau_{e} = \mNumber{2}/n\) for all edges, \(ell_{u}^{R} = (r_{t}) \ast (\mNumber{2}/n)\), and:
...
For general graphs at \(M_{t} = \mNumber{0}\) with the leverage filter (\(ell_{u} \le \mNumber{8}/\epsilon\)):

\[dbar_{t} \le (\mNumber{8} \ast t) / (\epsilon^\mNumber{2} \ast r_{t}).\]

At \(t \le \epsilon \ast |I_{\mNumber{0}}'|/\mNumber{3}\) and \(r_{t} \ge |I_{\mNumber{0}}'|(\mNumber{1} \mBridgeOperator{-} \epsilon/\mNumber{3}) \ge \mNumber{2}|I_{\mNumber{0}}'|/\mNumber{3}\):
...
structure (as in the \(K_{n}\) case where dbar \(=\) 2t/(n * \(\epsilon) \ll \mNumber{1}).\)

\textbf{Refined bound using total leverage:}

\[\begin{aligned}
\sum_{u \in S_{t}} ell_{u}^{R} \le \sum_{e \in E_{\text{cross}}} \tau_{e} \\
\le \sum_{e \in E(I_\mNumber{0})} \tau_{e} \le n \mBridgeOperator{-} \mNumber{1}.
\end{aligned}\]

So \(dbar_{t} \le\) (n-1)/(\(\epsilon \ast r_{t}\)). With \(r_{t} \ge \epsilon \ast n/\mNumber{4}\):
...
\(K_{n}, C_{n},\) Barbell, DisjCliq, ER(n,p) for \(n \in [\mNumber{8},\mNumber{64}]\) and \(\epsilon \in \{0.12, 0.15, 0.2, 0.25, 0.3\}\):
...
characteristic polynomial \(Q(x) = (\mNumber{1}/r) \sum_{v}\) det(xI - \(Y_{t}\)(v)):
...
\(S \subset I_\mNumber{0}'\) with \(|S| = T\) and \(M_{S} \prec \epsilon \ast I\) (i.e., \(L_{S} \le \epsilon \ast L\)).
...
\(|I_\mNumber{0}| = \Theta(\epsilon \ast n)\). Running the greedy for \(\Theta(\epsilon \ast |I_{\mNumber{0}}|)\)
steps gives \(|S| = \Theta(\epsilon^\mNumber{2} \ast n)\). This is inherent in the
heavy-edge-avoidance approach.

\textbf{For fixed \(\epsilon\) (the practical case):} With \(\epsilon = \mNumber{0}\).3:
\(|S| \ge \mNumber{0}.\mNumber{09} \ast n/\mNumber{36} = n/\mNumber{400}.\) With \(\epsilon = \mNumber{0}\).2: \(|S| \ge n/\mNumber{900}\).
...
\(P(|S| \ge \epsilon \ast n/\mNumber{6}) \ge \mNumber{1} \mBridgeOperator{-} \exp(-\epsilon \ast n/\mNumber{18})\). Numerically:
\(P(||M_{S}|| \le \epsilon \land |S| \ge \epsilon \ast n/\mNumber{6}) > \mNumber{0}\) for all tested
graphs (\(n \le \mNumber{80}\), 11 families, 4 \(\epsilon\) values, 500 trials each).
Success probability ranges from 0.2\% to 57\%.

This gives \(|S| \ge \epsilon \ast n/\mNumber{6} (c = \mNumber{1}/\mNumber{6}) but \text{the} formal matrix\)
...
\item
  ...
  \(||Y_{t}(v)|| < \mNumber{1} at \text{each} step\). Universal \(c = \mNumber{1}/\mNumber{3}\).
\item
...
\item
  ...
  Max \(dbar = \mNumber{0}.\mNumber{641} (K_{\mNumber{60}}, \epsilon = \mNumber{0}.\mNumber{3}, t = \mNumber{5}).\) Margin above 0: 36\%.
\item
...
\item
  ...
  \(size \ge \epsilon \ast n/\mNumber{6}\) for all tested graphs (\(n \le \mNumber{80}\), 272 combos).
\end{enumerate}
...
tr(\(H_{t}^{-\mNumber{1}} M_{\text{cross}})\) where \(H_{t} = \epsilon \ast I \mBridgeOperator{-} M_{t}\). When \(||M_{t}||\) is
...
structure (uniform \(\tau_{e} = \mNumber{2}/n\)) keeps dbar small.

For general graphs, the greedy\textquotesingle s selection criterion (\(\min \|Y_{t}(v)\|\))
...
The remaining gap is not \texttt{we\ don\textquotesingle{}t\ know\ enough\ math\textquotesingle{}\textquotesingle{};\ it\ is\ }we are
near the limit of this proof architecture.`` The architecture (barrier
greedy + PSD trace bound + pigeonhole) is correct --- it proves \(K_{n}\)
exactly and works numerically on every tested graph. The limit is that
the amplification factor when \(M_{t} \neq \mNumber{0}\) makes the naive bound on dbar
exceed 1, while the greedy\textquotesingle s self-correcting property (selecting
vertices whose contributions are orthogonal to \(M_{t}'s\) large eigenspace)
is not yet captured by the formal analysis.

Empirically, the self-correction is strong: the spectral amplification
factor is 0.52 (vs the scalar worst-case of 1.0), and the \(W-M_{t}\)
alignment \(\text{is} \le \mNumber{0}\).25 across all 351 Phase 2 steps. Closing the gap
...
\item
  For PSD Y: \(\|Y\| \le \mOpName{tr}(Y)\) (spectral norm bounded by trace)
\item
  Pigeonhole: \(min_{v} f(v) \le (\mNumber{1}/r) \sum_{v} f(v) (minimum \le average)\)
\item
  Turan: independence number \(\ge n^{\mNumber{2}}/(\mNumber{2}m+n)\)
\item
  For \(K_{n}\): \(\tau_{e} = \mNumber{2}/n, ||M_{S}|| = |S|/n (\text{exact})\)
\end{enumerate}
...
\item
  ...
  Review 56(2), 315-334.
\item
  Marcus, Spielman, Srivastava (2015), \(Interlacing Families II: Mixed Characteristic Polynomials \text{and the} Kadison-Singer Problem,\) Annals of
  Mathematics 182(1), 327-350.
\item
  Borcea, Branden (2009), \(\text{The} Lee-Yang \text{and} Polya-Schur programs. I. Linear operators preserving stability,\) Inventiones Math. 177, 541-569.
\item
