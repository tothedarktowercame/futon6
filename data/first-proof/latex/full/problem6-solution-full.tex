...
concentration bound, one would need \(R_* \le C_\mNumber{1} \ast \epsilon\) and
\textbar\textbar{}\(W_{n}\)\textbar\textbar{} \textless= \(C_\mNumber{2} \ast \epsilon^\mNumber{2}\) for graph-dependent constants \(C_\mNumber{1},\) \(C_\mNumber{2}.\) Bounding
...
\[\begin{aligned}
exists c\mNumber{0}>\mNumber{0} universal such that \\
for all G, \epsilon \in (\mNumber{0},\mNumber{1}), exists S with |S|\ge c\mNumber{0} \ast \epsilon \ast n and L_{S}\le \epsilon L.
\end{aligned}\]
...
There exists c\mNumber{0}>\mNumber{0} such that for every weighted graph G and \epsilon \in (\mNumber{0},\mNumber{1}), \\
one can choose S subseteq V with |S| \ge c\mNumber{0} \ast \epsilon \ast n and L_{S} \le \epsilon L.
\end{aligned}\]
...
\item
  \(L^{+/\mNumber{2}}\) \(L_{S}\) \(L^{+/\mNumber{2}} \le sum_{v}\) \(Z_{v}\) \(A_{v}\) with \(A_{v}\)=(1/2)\(sum_{u~v}\)\(X_{uv}\)
\end{enumerate}
