...
\(\epsilon\) in (0,1), there exists S with \textbar S\textbar{} \textgreater= c\emph{\(\epsilon\)}n and \(L_{S}\) \textless= \(\epsilon\) L?
...
\[for all x \in R^{V}: x^{T} L_{S} x \le \epsilon x^{T} L x.\]

On im(L), with \(L^+\) the Moore-Penrose pseudoinverse:

\[L_{S} \le \epsilon L \Leftrightarrow || L^{+/2} L_{S} L^{+/2} || \le \epsilon.\]
...
For G=\(K_{n}\) and S of size s, choose x supported on S with \(sum_{i \in S}\) \(x_{i}\) = 0.
Then
...
Hence \(L_{S}\) \textless= \(\epsilon\) \(L_{K_{n}}\) implies s \textless= \(\epsilon\) n.
...
\[E[\epsilon L - L_{S}] = (\epsilon - p^2)L.\]

Setting p=epsilon gives E{[}\(L_{S}\){]}=\(\epsilon^2\) L \textless= \(\epsilon\) L. This is not yet a
...
\[X_{e} = L^{+/2} w_{e} b_{e} b_{e}^{T} L^{+/2}, b_{e} = e_{u}-e_{v},\]

and leverage scores

\[\begin{aligned}
\tau_{e} = tr(X_{e}) = w_{e} b_{e}^{T} L^+ b_{e}, \\
sum_{e} \tau_{e} = n - k
\end{aligned}\]
...
Using \(Z_{u}\) \(Z_{v}\) \textless= \(Z_{u}\) and \(Z_{u}\) \(Z_{v}\) \textless= \(Z_{v}\) for each edge \{u,v\},

\[\begin{aligned}
L_{S} = sum_{uv \in E} Z_{u} Z_{v} L_{uv} \\
\le (1/2) sum_{v} Z_{v} sum_{u~v} L_{uv}.
\end{aligned}\]

So in normalized coordinates

\[\begin{aligned}
L^{+/2} L_{S} L^{+/2} \le sum_{v} Z_{v} A_{v}, \\
A_{v} := (1/2) sum_{u~v} X_{uv} \ge 0.
\end{aligned}\]
...
With filtration \(F_{i}\) = \(\sigma(Z_1,...,Z_{i})\), Doob martingale

\[\begin{aligned}
Y_{i} = E[X | F_{i}], \\
\Delta_{i} = Y_{i} - Y_{i-1}
\end{aligned}\]
...
||\Delta_{i}|| \le ||A_{i}|| \le R_ \ast (R_ * = max_{i} ||A_{i}||),
\end{aligned}\]

and predictable quadratic variation

\[W_{n} = sum_{i} E[\Delta_{i}^2 | F_{i-1}] = sum_{i} p_{i}(1-p_{i}) A_{i}^2.\]

Matrix Freedman (or matrix Bernstein in independent form) applies once bounds
on \(R_*\) and \textbar\textbar{}\(W_{n}\)\textbar\textbar{} are supplied.

\textbf{What graph-dependent bounds are needed.} To obtain a self-contained
concentration bound, one would need \(R_*\) \textless= \(C_1 \ast \epsilon\) and
\textbar\textbar{}\(W_{n}\)\textbar\textbar{} \textless= \(C_2 \ast \epsilon^2\) for graph-dependent constants \(C_1,\) \(C_2.\) Bounding
...
method controls both \(R_*\) and \textbar\textbar{}\(W_{n}\)\textbar\textbar{} simultaneously through a potential
...
\hypertarget{5-external-dependency-for-universal-c00}{%
\subsection{5. External dependency for universal c0\textgreater0}\label{5-external-dependency-for-universal-c00}}
...
for all G, \epsilon \in (0,1), exists S with |S|\ge c0 \ast \epsilon \ast n and L_{S}\le \epsilon L.
\end{aligned}\]
...
one can choose S subseteq V with |S| \ge c0 \ast \epsilon \ast n and L_{S} \le \epsilon L.
\end{aligned}\]
...
for a \emph{vertex subset} S with \(L_{S}\) \textless= \(\epsilon\) L. These are different objects:
...
\item
  \(L_{S}\) = \(sum_{e internal to S}\) \(w_{e}\) \(b_{e}\) \(b_{e}^{T}\)
\item
  \(\tau_{e}\) = \(w_{e}\) \(b_{e}^{T}\) \(L^+\) \(b_{e},\) \(sum_{e}\) \(\tau_{e}\) = n-k
\item
  \(L^{+/2}\) \(L_{S}\) \(L^{+/2}\) \textless= \(sum_{v}\) \(Z_{v}\) \(A_{v}\) with \(A_{v}\)=(1/2)\(sum_{u~v}\)\(X_{uv}\)
\end{enumerate}
