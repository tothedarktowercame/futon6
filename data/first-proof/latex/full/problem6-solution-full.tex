...
\[\begin{aligned}
\text{Assumption} V (\text{vertex}-\text{light} \text{selection} \text{theorem}): \\
\text{There} \text{exists} c\mNumber{0} > \mNumber{0} \text{such} \text{that} \text{for} \text{every} \text{weighted} \text{graph} G \text{and} \epsilon \in (\mNumber{0},\mNumber{1}), \\
\text{one} \text{can} \text{choose} S \subseteq V \text{with} |S| \ge c\mNumber{0} \ast \epsilon \ast n \text{and} L_{S} \le \epsilon L.
\end{aligned}\]
