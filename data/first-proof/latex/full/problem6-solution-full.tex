...
Setting p=epsilon gives E{[}\(L_{S}\){]}=\(\epsilon^2\) \(L \le \epsilon\) L. This is not yet a
...
concentration bound, one would need \(R_* \le C_1 \ast \epsilon\) and
...
\item
  \(L^{+/2}\) \(L_{S}\) \(L^{+/2} \le sum_{v}\) \(Z_{v}\) \(A_{v}\) with \(A_{v}\)=(1/2)\(sum_{u~v}\)\(X_{uv}\)
\end{enumerate}
