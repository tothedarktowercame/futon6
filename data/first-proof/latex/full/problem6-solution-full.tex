...
Because \(Z_{v}\) are independent Bernoulli variables, the random matrices \(Z_{v} A_{v}\)are independent PSD summands.
...
on \(R_{\mDualStar}\) and \textbar\textbar{}\(W_{n}\)\textbar\textbar{} are supplied.
...
\textbar\textbar{}\(W_{n}|| \le C_{\mNumber{2}} \ast \epsilon^{\mNumber{2}}\)for graph-dependent constants \(C_{\mNumber{1}}, C_{\mNumber{2}}.\)Bounding
...
method controls both \(R_{\mDualStar}\) and \textbar\textbar{}\(W_{n}\)\textbar\textbar{} simultaneously through a potential
