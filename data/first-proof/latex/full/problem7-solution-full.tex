...
\(H_{\mDualStar}(M_{tilde}; Q) = \mNumber{0}\) for \(* > \mNumber{0}\). Obligation E2 gives a finite CW
complex with these properties; the upgrade to a closed manifold was the
remaining obstacle.
...
\texttt{Y} with \(\pi_\mNumber{1}(Y) = \Gamma\) and \(H_{\mDualStar}(Y_{tilde}; Q) = \mNumber{0}\) for \(* > \mNumber{0}\).
...
\(* > \mNumber{0}\). The manifold is constructed by equivariant surgery (S-rot-II):
...
\item
  ...
  \item
    ...
    to be rationally hyperbolic → θ ⊗ \(Q = \mNumber{0}\).
  \item
  ...
  ∂W = S(ν) = F \(\times\) S¹ and free Z/2-action on W. Since θ = 0, a cap V
  exists with ∂V = F \(\times\) S¹ and free Z/2-action. Set M\textquotesingle{} = W ∪ V. Then
  N = M\textquotesingle/(Z/2) is a closed manifold with \(\pi\)₁(N) = Γ and rationally
