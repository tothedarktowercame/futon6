...
orbifold extension group \(\pi_\mNumber{1}((EG \times Y)/G)\) lies in \(FH(Q)\).
...
\item
  ...
  \(\Gamma_\mNumber{0} < Isom(H^{n})\) containing a reflection \(\tau\), as provided by
...
\item
  \textbf{Extension.} Set \(G = <\tau_{bar}> = Z/\mNumber{2}\) acting on \(Bpi := M\).
...
\item
  ...
  \((n+\mNumber{1}, \mNumber{0})\). So \(SO(f)\) gives \(SO(n, \mNumber{1})\) at one place and a compact
  group at the other.
\item
  \textbf{Uniform lattice.} \(\Gamma_\mNumber{0} = SO(f, O_{k})\) is a cocompact arithmetic
  lattice in \(SO(n, \mNumber{1})\) (Borel-Harish-Chandra; cocompactness by Godement
...
\item
  ...
  \(SO(f, O_{k})\): it preserves \texttt{f} (negates \(x_\mNumber{1}, x_\mNumber{2}\) in the \(x_\mNumber{1}^\mNumber{2}+x_\mNumber{2}^\mNumber{2}\)
...
\item
  ...
  \(\chi = \mNumber{0}\). By Fowler\textquotesingle s Main Theorem: \(\Gamma \in FH(Q)\).
\end{enumerate}
