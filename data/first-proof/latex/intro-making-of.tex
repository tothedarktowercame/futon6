...
\begin{itemize}[leftmargin=1.5em]
\item Problems 1, 2, 7, 8, and 9 are currently treated as \emph{provisionally closed in manuscript form}; a full node-level validation sweep is still pending.
\item Problem 3 now has a largely validated existence path (current run: 3 verified, 5 plausible) with one remaining validator gap in the optional uniqueness/irreducibility layer; for quality control it remains provisional until that final validation gap is closed.
