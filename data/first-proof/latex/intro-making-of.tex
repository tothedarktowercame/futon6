...
\item[AIF] Argument Interchange Format---a graph-based standard for representing argumentation structure (nodes = claims/moves, edges = inferential/conflict/preference relations).
\item[BSS] Batson--Spielman--Srivastava spectral sparsification framework.
\item[IATC] Inference Anchoring Theory + Content---an annotation scheme for mathematical dialogues, extending AIF with content-level anchoring~\cite{corneli2017}.
...
\begin{itemize}[leftmargin=1.5em]
\item Problem 6 is proved for $K_n$ ($c = 1/3$) via an elementary barrier-greedy argument; a narrow technical gap remains for general graphs (formal $\bar{d} < 1$ bound at $M_t \ne 0$).
