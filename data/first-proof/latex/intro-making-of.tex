...
\begin{itemize}[leftmargin=1.5em]
\item \textbf{P1 [C|\(\sim\)].} Mathematical claim is treated as closed; QC uncertainty is mainly verification-track alignment (unified repair vs.\ legacy node ledger). See \hyperref[sn:p1]{Status Note P1} (p.~\pageref{sn:p1}).
\item \textbf{P2 [C|\(\sim\)].} Local algebraic chain is treated as repaired; result still depends on the cited newvector/test-vector theorem input and ledger alignment. See \hyperref[sn:p2]{Status Note P2} (p.~\pageref{sn:p2}).
...
\item \textbf{P4 [P|?].} Proved analytically for $n \le 3$; the $n=4$ branch has strong computational support with one certification step pending; higher $n$ remains open. See \hyperref[sn:p4]{Status Note P4} (p.~\pageref{sn:p4}).
\item \textbf{P5 [C|\(\sim\)].} Closed in the declared local scope for $F_O$-local connective spectra; indexing-system-level extension remains open. See \hyperref[sn:p5]{Status Note P5} (p.~\pageref{sn:p5}).
\item \textbf{P6 [P|?].} $K_n$ case proved ($c_0 = 1/3$); one technical gap remains in the general-graph argument (formal $\bar{d} < 1$ bound at $M_t \ne 0$). See \hyperref[sn:p6]{Status Note P6} (p.~\pageref{sn:p6}).
\item \textbf{P7 [C|\(\sim\)].} Provisionally closed mathematically via the rotation route; independent theorem-ledger re-check remains the main QC holdout. See \hyperref[sn:p7]{Status Note P7} (p.~\pageref{sn:p7}).
\item \textbf{P8 [C|\(\sim\)].} Geometric closure is treated as repaired, with remaining risk concentrated in canonical ledger synchronization. See \hyperref[sn:p8]{Status Note P8} (p.~\pageref{sn:p8}).
\item \textbf{P9 [P|\(\sim\)].} Strong computational evidence exists, but the universal converse step is not yet cleanly closed algebraically. See \hyperref[sn:p9]{Status Note P9} (p.~\pageref{sn:p9}).
\item \textbf{P10 [?|?].} Conditional under explicitly stated assumptions; assumption necessity is explicit, while final closure/validation remains open. See \hyperref[sn:p10]{Status Note P10} (p.~\pageref{sn:p10}).
