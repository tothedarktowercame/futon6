...
\subsubsection{\texorpdfstring{Interpreting \(\Phi_{n}\) algebraically}{Interpreting Phi\_n algebraically}}\label{interpreting-phi_n-algebraically}}
...
\subsubsection{Connection to the discriminant}\label{connection-to-the-discriminant}}
...
\subsubsection{Finite free convolution and root behavior}\label{finite-free-convolution-and-root-behavior}}
...
\subsubsection{The inequality via the random matrix model}\label{the-inequality-via-the-random-matrix-model}}
...
\subsubsection{Finite free cumulants and the bilinear structure}\label{finite-free-cumulants-and-the-bilinear-structure}}
...
\subsubsection{\texorpdfstring{Complete proof for \(n = \mNumber{3}\)}{Complete proof for n = 3}}\label{complete-proof-for-n-3}}
...
Combining: surplus \(\ge\) 0. Equality iff \(u/s^\mNumber{2} = v/t^\mNumber{2}\) and \(s = t\) or \(u = v = \mNumber{0}\).

\textbf{QED for \(n = \mNumber{3}\).}

\hypertarget{5b-what-the-proof-requires-for-n--4-gap}{%
\subsubsection{\texorpdfstring{What the proof requires for \(n \ge \mNumber{4}\)}{What the proof requires for n >= 4}}\label{what-the-proof-requires-for-n-ge-4}}
...
(b) \(\boxplus_\mNumber{4}\) has a cross-term even for centered polynomials: \(c_{\mNumber{4}} = a_{\mNumber{4}} + (\mNumber{1}/\mNumber{6}) \ast a_{\mNumber{2}} \ast b_{\mNumber{2}} \mBridgeOperator{+} b_{\mNumber{4}}.\)
...
\subsubsection{Verification for small cases}\label{verification-for-small-cases}}
...
\statusnote{The inequality is conjecturally true, with strong numerical
evidence and analytic proofs for $n = 2$, $3$.
$n = 2$: proved (equality; $1/\Phi_2$ is linear in the discriminant).
$n = 3$: proved (strict inequality; $\Phi_3 \cdot \mathrm{disc} = 18\,a_2^2$
identity + Titu's lemma).
$n = 4$: numerically verified (0/3000 violations; the $\boxplus_4$
cross-term $a_2 b_2/6$ is essential).
$n = 5$: numerically verified (0/2000 violations).
What is established analytically:
(i)~finite free cumulants add under $\boxplus_n$ (Arizmendi--Perales 2018);
(ii)~$\boxplus_n$ commutes with translation (random matrix argument);
(iii)~superadditivity is specific to $\boxplus_n$---plain coefficient
addition fails ${\sim}40\%$ of the time.}

\begin{openobligation}[title={Open Obligation --- What Remains Open for $n \ge 4$}]
\textbf{What remains open for \(n \ge \mNumber{4}\):}

The \(n = \mNumber{3}\) proof relies on \(\Phi_\mNumber{3}\) * disc \(= \mNumber{18} \ast a_{\mNumber{2}}^{\mNumber{2}}\) being constant in \(a_\mNumber{3}\).
This fails at \(n = \mNumber{4}\) (the product depends on all coefficients). The \(\boxplus_\mNumber{4}\)
cross-term (1/6) * \(a_\mNumber{2} \ast b_\mNumber{2} \in c_\mNumber{4}\) is essential (plain addition fails 29\%).
A proof for \(n \ge \mNumber{4}\) needs either:
...
\end{openobligation}
\begin{processnote}[title={Process Note --- Numerical Evidence}]
%
...
Ratio \(= LHS/RHS = [\mNumber{1}/\Phi_{n}(p\boxplus q)] / [\mNumber{1}/\Phi_{n}(p) \mBridgeOperator{+} \mNumber{1}/\Phi_{n}(q)]\); ratio \(\ge\) 1
...
\end{processnote}
\begin{processnote}[title={Process Note --- futon6 Corpus References}]
%
