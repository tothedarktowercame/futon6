...
Problems 1, 2, 7, 8, and 9 are currently treated as provisionally closed in
manuscript form, pending full node-level validation; Problem 3 now has a
largely validated existence path with one remaining validator gap in the
optional uniqueness layer and is therefore still provisional; Problem 10 is
conditional under clearly stated assumptions, with
explicit necessity counterexamples when those assumptions are dropped; Problems 4 and
6 are partial, with named remaining obligations in the general case (including
an unconditional $K_n$ result for Problem 6 with $c_0 = 1/3$); and Problem 5 is
solved in a scope-limited form for $F_O$-local connective spectra. The main
methodological result is that structure-aware verification and layer-switching
...
\textbf{Closed}~(considered fully solved);
\textbf{Partial}~(proved in key cases, with explicit remaining obligations);
\textbf{Conditional}~(closed under clearly stated assumptions).

Current outcome summary: Problems 1, 2, 7, 8, and 9 are provisionally closed in manuscript form pending validation sweep; Problem 3 has a largely validated existence path with one remaining validator gap in an optional uniqueness layer (still provisional); Problem 10 is conditional under stated assumptions (with explicit necessity counterexamples for assumption violations); Problem 5 is solved in a scope-limited form; and Problems 4 and 6 are partial with explicit remaining obligations in the general case.
