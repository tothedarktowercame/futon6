...
\section{Summary}
At a structural level, the argument is simple: generation is fast, while
validation and stabilization are slower; git logs record states and transitions
but not argumentative relations; those relations can be typed and represented
explicitly as a graph; once the structure is explicit, stagnation and policy
shifts become machine-detectable; and once stagnation is detectable, automated
metacognitive interrupts become possible.  The design conclusion follows:
proof strategy should be represented as a first-class typed graph object.
