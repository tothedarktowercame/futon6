...
\section{From Failure to Success: Problem 6's Layer Switch}
...
\item \textbf{Leverage filter} removes high-leverage vertices,
  keeping $|I_0'| \ge \varepsilon n/12$.
...
For general graphs: $\bar{d}_t < 1$ is proved at $M_t = 0$ and verified
numerically at all 440 nontrivial greedy steps (max $\bar{d} = 0.641$,
margin $36\%$).  The single remaining gap is the formal
$\bar{d}_t < 1$ bound when $M_t \ne 0$.

No interlacing families.  No Borcea--Br\"and\'en.  No MSS theorem.
Just PSD trace bound $+$ pigeonhole $+$ Tur\'an.
...
\item[Concentration layer:] Control martingale parameters $R_*$
  and $\|W_n\|$.  \emph{Status: framework correct (unchanged).}
\item[Combinatorial layer:] Tur\'an $+$ leverage filter $+$ barrier greedy.
  \emph{Status: \textbf{complete} for $K_n$; narrow technical gap for
  general graphs.}
