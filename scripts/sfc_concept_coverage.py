...
def resolved_genuine_concept(
    ...
    *,
    ...
    if action == "HAPAX":
        return None, action
    if action == "OVERFED":
        candidate = normalize_concept(str(resolved.get("resolution") or ""))
    else:
        candidate = phrase
...
def genuine_ranking(
    ...
    *,
    ...
    for phrase, count in sorted(df.items()):
        concept, action = resolved_genuine_concept(phrase, df, min_papers=min_papers)
        if concept is None:
            continue
...
def definition_sets(
    ...
    for concept, rows in (snippets.get("snippets") or {}).items():
        if rows:
            sources.setdefault(normalize_concept(concept), set()).add("def-snippets")

    for concept, papers in (defined_index.get("concept_to_papers") or {}).items():
        if papers:
            sources.setdefault(normalize_concept(concept), set()).add("defined-index")

    for entry in encyclopedia.get("entries") or []:
        if not isinstance(entry, dict) or not entry.get("concept"):
            continue
        concept = normalize_concept(str(entry["concept"]))
    ...
    *,
    ...
    for summary in summaries:
        ...
        lines.append(
            ...
            f"{summary['coverage']:.1%} | {undefined_count} |"
        )
