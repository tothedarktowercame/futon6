...
NODE_GAP_SPECS: dict[str, dict] = {
    ...
    # =========================================================================
    # P2: Rankin-Selberg / representation theory (REVIEWER.md: 2 critical gaps)
    # =========================================================================
    "p2-problem": {
        "topic": "existence of test vectors for Rankin-Selberg integrals",
        "gap": "The fixed-W universality claim is the core unproved step. Need QA "
               "about when a single Whittaker function W can serve as a universal "
               "test vector across all generic pi of GL_n.",
        "seed_tags": ["representation-theory", "number-theory", "automorphic-forms"],
    },
    "p2-s3": {
        "topic": "u_Q twist and nonvanishing of restricted Whittaker functions",
        "gap": "R(u_Q)W nonzero does not automatically imply phi_Q nonzero on embedded "
               "GL_n. Need QA about restriction of Whittaker functions to Levi subgroups "
               "and when nonvanishing is preserved under embedding.",
        "seed_tags": ["representation-theory", "number-theory", "lie-groups"],
    },
    "p2-s3a": {
        "topic": "spanning lemma for fractional ideals of L-functions",
        "gap": "The jump from ideal language to single-V monomial output is not justified. "
               "Need QA about JPSS theory, fractional ideals generated by zeta integrals, "
               "and when a single vector suffices.",
        "seed_tags": ["number-theory", "representation-theory", "l-functions"],
    },
    "p2-s5": {
        "topic": "conductor matching and epsilon factor constraints",
        "gap": "Conductor matching section is heuristic. Need QA about Godement-Jacquet "
               "vs Rankin-Selberg conductor exponents and when they agree.",
        "seed_tags": ["number-theory", "automorphic-forms", "l-functions"],
    },
    # =========================================================================
    # P3: Markov chains on symmetric functions (REVIEWER.md: 2 major gaps)
    # =========================================================================
    "p3-problem": {
        "topic": "finite-state CTMC with modified Macdonald polynomial stationary law",
        "gap": "The star/non-star normalization bridge (F* vs F) is asserted, not proved. "
               "Need QA about when the starred vs unstarred normalization conventions "
               "for modified Macdonald polynomials agree and their ratio.",
        "seed_tags": ["combinatorics", "probability", "symmetric-functions"],
    },
    "p3-s1": {
        "topic": "multispecies t-PushTASEP construction on ring sites",
        "gap": "The explicit chain construction uses truncated geometric probabilities "
               "that need justification. Need QA about how t-geometric weighting arises "
               "from the Macdonald polynomial structure.",
        "seed_tags": ["combinatorics", "probability", "markov-chains"],
    },
    "p3-s4": {
        "topic": "AMW theorem specialization to q=1 ring model",
        "gap": "Irreducibility of the chain is claimed but not explicitly established. "
               "Need QA about when multispecies exclusion processes are irreducible "
               "on fixed-content sectors.",
        "seed_tags": ["probability", "markov-chains", "combinatorics"],
    },
    "p3-s6": {
        "topic": "positivity of stationary weights in parameter range",
        "gap": "Positivity of F_mu(x;1,t)/P_lambda(x;1,t) for x_i>0, 0<=t<1 needs "
               "direct citation. Need QA about when modified Macdonald polynomials "
               "have positive coefficients.",
        "seed_tags": ["combinatorics", "symmetric-functions", "algebraic-combinatorics"],
    },
    # =========================================================================
    # P8: Lagrangian smoothing (REVIEWER.md: 2 critical gaps)
    # =========================================================================
    "p8-problem": {
        "topic": "Lagrangian smoothing of polyhedral surfaces in R^4",
        "gap": "The basis/nondegeneracy claim at vertices is not justified by the "
               "manifold condition alone. Need QA about when 4-face vertex stars in "
               "polyhedral Lagrangians admit nontrivial symplectic direct sum decomposition.",
        "seed_tags": ["symplectic-geometry", "differential-topology", "manifolds"],
    },
    "p8-s3": {
        "topic": "symplectic direct sum decomposition at vertices",
        "gap": "The vertex spanning lemma proof is algebraic but its topological "
               "prerequisites are unstated. Need QA about when omega block-diagonalizes "
               "in coordinates adapted to a Lagrangian fan.",
        "seed_tags": ["symplectic-geometry", "linear-algebra", "differential-geometry"],
    },
    "p8-s5": {
        "topic": "local surgery via Hamiltonian isotopy",
        "gap": "The local surgery theorem is invoked outside its stated smooth hypotheses. "
               "Need QA about extending Lees/Weinstein neighborhood theorems to "
               "polyhedral/piecewise-linear Lagrangians.",
        "seed_tags": ["symplectic-geometry", "differential-topology", "surgery-theory"],
    },
    "p8-s6": {
        "topic": "global patching of local Hamiltonian isotopies",
        "gap": "Local isotopies at each vertex/edge must be patched into a global "
               "smoothing. Need QA about when local Hamiltonian isotopies extend "
               "globally, and what obstructions arise.",
        "seed_tags": ["symplectic-geometry", "differential-topology", "manifolds"],
    },
}
...
def identify_gaps(
    ...
    # Determine problem prefix from wiring thread_id (e.g., "first-proof-p3" → "p3")
    problem_prefix = wiring.get("thread_id", "").replace("first-proof-", "")

    for node_id, gap_spec in NODE_GAP_SPECS.items():
        # Only include gap specs that match this problem's node prefix
        if not node_id.startswith(problem_prefix + "-"):
            continue
...
def main():
    ...
    # Load corpus context (optional — dry-run works without it)
    ...
    if not corpus_context:
        if not args.dry_run:
            ...
            return 2
        print(f"No corpus context found — proceeding without exemplar threads")
        corpus_context = {}
