...
def emit_edn(moves: list[dict], metric_name: str = "option-a", out_path=None) -> None:
    out_path = out_path or MOVES
    edn = (";; diffsub-moves.edn — M-differentiable-substrate gradient-scored move-set.\n"
           ...
           f" :emit/metric {{:compose :additive :epistemic :C-holes :pragmatic :cap-ascent :C-variant :{metric_name}}}\n"
           f" :emit/k {len(moves)}\n"
           " :emit/stub? false\n"
           " :moves [\n" + "\n".join(edn_move(d) for d in moves) + "\n ]}\n")
    out_path.write_text(edn)


# --- hot-swappable epistemic-metric registry (Joe 2026-06-09: side-by-side, don't fix blind) ---
# Each variant maps a scope node's signals -> g(s); capability nodes always use capability_metric
# (the pragmatic pole). Swap at --jax time (no re-embed); the choice is tagged in :emit/metric
# :C-variant. Structure (moves/chains/roots) is identical across variants — only :score/:prior
# differ — so claude-4's handshake holds for every variant; the experiment compares prior quality.
def _scope_signals(nd: dict, ctx: dict):
    det = 1.0 if nd.get("det") else 0.0
    frontier = 1.0 if nd.get("binder") in FRONTIER_BINDERS else 0.0
    classw = CLASSW.get(nd.get("class"), 0.45)
    gen = float(ctx["gen"].get(nd.get("mission"), 0))
    degree = float(nd.get("degree", 0.0))
    return det, frontier, classw, gen, degree


def metric_option_a(nd, ctx):   # baseline: det + frontier + class (sparse / flat)
    det, fr, cw, gen, deg = _scope_signals(nd, ctx)
    return (0.18 + det + 0.30 * fr) * cw


def metric_sharp(nd, ctx):      # + generativity + degree + wider class spread (more variance)
    det, fr, cw, gen, deg = _scope_signals(nd, ctx)
    return (0.18 + det + 0.30 * fr + 0.10 * math.log1p(gen) + 0.04 * math.log1p(deg)) * (0.35 + 1.3 * cw)


def metric_liveness(nd, ctx):   # Salingaros-class-dominant (mess/alive drives g(s))
    det, fr, cw, gen, deg = _scope_signals(nd, ctx)
    return (0.10 + 0.5 * det + 0.20 * fr) * (0.20 + 2.0 * cw)


METRICS = {"option-a": metric_option_a, "sharp": metric_sharp, "liveness": metric_liveness}


def metric_context() -> dict:
    return {"gen": mission_generativity()}


def compute_metric_array(nodes, metric_name, ctx, caps):
    fn = METRICS[metric_name]
    return np.array([capability_metric(nd["cap"], caps.get(nd["cap"], {}), caps)
                     if nd.get("kind") == "capability" else fn(nd, ctx)
                     for nd in nodes], dtype=np.float32)


def do_jax(grain: str, metric_name: str = "option-a", out_path=None, announce: bool = True) -> dict:
    import jax
    ...
    ctx = metric_context()
    metric = compute_metric_array(nodes, metric_name, ctx, caps)
    ...
    moves = moves[:TOP_K]
    # PRIOR: z-normalize scores then apply temperature. The raw gradient scores are small-
    # magnitude (gnorm ~1e-5), so a plain softmax flattens real ranking signal (score-cv ~0.36)
    # into a uniform prior — useless to PUCT. z-norm makes the prior reflect the relative RANKING
    # (the signal that's actually there); temperature controls peakedness. (Found via the metric
    # side-by-side: uniform across all variants ⇒ the lever is here, not the metric weights.)
    PRIOR_TEMP = float(os.environ.get("DIFFSUB_PRIOR_TEMP", "2.0"))
    _raw = np.array([m["score"] for m in moves], dtype=np.float64)
    _z = (_raw - _raw.mean()) / (_raw.std() + 1e-9)
    priors = softmax(list(_z * PRIOR_TEMP))
    ...
    out_path = out_path or MOVES
    emit_edn(moves, metric_name, out_path)
    ...
    # prior discrimination — a USEFUL policy prior is PEAKED (low entropy / high spread),
    # not flat; a flat prior gives claude-4's PUCT no guidance. These are the side-by-side numbers.
    pr = np.array(priors) if priors else np.array([1.0])
    entropy = float(-np.sum(pr * np.log(pr + 1e-12)))
    max_entropy = float(np.log(len(pr))) if len(pr) > 1 else 1.0
    sc = np.array([m["score"] for m in moves]) if moves else np.array([0.0])
    summary = {
        "metric": metric_name,
        ...
        "prior_entropy_norm": (entropy / max_entropy) if max_entropy else 0.0,
        "prior_max": float(pr.max()),
        "score_cv": float(sc.std() / (sc.mean() + 1e-12)),
    }
    summary_path.write_text(json.dumps(summary, indent=2))
    if announce:
        print(f"[metric={metric_name}] {summary['grain']} {summary['adjacency']}")
        print(f"[conditioning] max/med={summary['gradnorm_max_med_ratio']:.2f} corr(degree)={corr:+.3f} "
              f"sat {sat_before:.4f}->{sat_after:.4f}")
        print(f"[discrimination] prior-entropy-norm={summary['prior_entropy_norm']:.3f} (0=peaked/1=flat) "
              f"prior-max={summary['prior_max']:.4f} score-cv={summary['score_cv']:.3f}")
        print(f"[anchors] summit={summary['summit_score_med']:.6f} island={summary['island_score_med']:.6f}")
        print(f"wrote {out_path} — {len(moves)} moves")
    return summary


def run_experiment(grain: str) -> None:
    """Side-by-side: run every metric variant (same structure, different scores) and compare
    prior quality. The canonical diffsub-moves.edn stays = option-a (stable for claude-4's
    handshake); each variant also written to diffsub-moves-<name>.edn for rollout comparison."""
    print(f"=== metric side-by-side (grain={grain}) — lower entropy-norm + higher prior-max/score-cv = sharper, more useful prior ===")
    print(f"{'variant':10s} {'entropy-norm':>12s} {'prior-max':>10s} {'score-cv':>9s} {'sat->':>14s} {'summit/island':>16s}")
    for name in METRICS:
        out = HERE / f"data/diffsub-moves-{name}.edn"
        s = do_jax(grain, name, out, announce=False)
        print(f"{name:10s} {s['prior_entropy_norm']:>12.3f} {s['prior_max']:>10.4f} {s['score_cv']:>9.3f} "
              f"{s['sat_before']:.3f}->{s['sat_after']:.3f}  {s['summit_score_med']:.4f}/{s['island_score_med']:.4f}  -> {out.name}")
    do_jax(grain, "option-a", MOVES, announce=False)  # canonical = option-a (handshake-stable)
    print("\ncanonical data/diffsub-moves.edn = option-a; variants in data/diffsub-moves-<name>.edn")
    print("structure (55 moves / 21 mission roots / chains) is IDENTICAL across variants — only :score/:prior differ.")
...
def main() -> None:
    ...
    ap.add_argument("--metric", choices=list(METRICS), default="option-a")
    args = ap.parse_args()
    if args.embed:
        do_embed(args.grain)
    elif args.experiment:
        run_experiment(args.grain)
    elif args.jax:
        do_jax(args.grain, args.metric)
    else:
        ap.error("use --embed then --jax [--metric NAME] | --experiment")
